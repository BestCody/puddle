...
test('configured Supabase host is approved for managed photos', () => {
  ...
  assert.equal(approvedPhotoUrl('https://project-ref.supabase.co/storage/v1/object/public/puddle-public-media/open-photos/a.jpg', hosts)?.hostname, 'project-ref.supabase.co')
})
...
test('photo metadata exposes attribution and managed storage details without remote provider URL', () => {
  const metadata = photoMetadata({
    ...
    storage_backend: 'supabase', storage_key: 'open-photos/by-hash/ab/abc.jpg', byte_size: 42000,
  ...
  assert.deepEqual(metadata, {
    ...
    storageBackend: 'supabase', storageKey: 'open-photos/by-hash/ab/abc.jpg', byteSize: 42000
  })
