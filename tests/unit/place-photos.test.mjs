...
test('photo URLs require exact approved HTTPS hosts', () => {
  const hosts = allowedPhotoHosts('images.example.com,cdn.example.com', null, null)
...
test('configured Supabase and Backblaze B2 hosts are approved for managed photos', () => {
  ...
  const hosts = allowedPhotoHosts(
    ...
    'https://f005.backblazeb2.com/file/puddle-assets'
  )
  assert.deepEqual([...hosts].sort(), ['f005.backblazeb2.com', 'images.example.com', 'project-ref.supabase.co'])
  assert.equal(approvedPhotoUrl('https://f005.backblazeb2.com/file/puddle-assets/photos/open/a.avif', hosts)?.hostname, 'f005.backblazeb2.com')
})
...
test('photo metadata exposes attribution and managed storage details without remote provider URL', () => {
  const metadata = photoMetadata({
    ...
    storage_backend: 'b2', storage_key: 'photos/open/ab/abc.avif', byte_size: 42000,
  ...
  assert.deepEqual(metadata, {
    ...
    storageBackend: 'b2', storageKey: 'photos/open/ab/abc.avif', byteSize: 42000
  })
