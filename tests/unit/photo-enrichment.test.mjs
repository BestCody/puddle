...
test('the active card waits for a private B2 grant before using Google Places UI Kit fallback', async () => {
  ...
  assert.ok(card.includes('!mainPhoto && !mainPhotoPending && Boolean(item.google_place_id || googleLookup)'))
  assert.ok(card.includes('lookup={googleLookup}'))
  ...
  assert.ok(googleFallback.includes("request.setAttribute('place', resolvedPlaceId)"))
