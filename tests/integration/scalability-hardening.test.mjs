...
test('active messaging uses cursor-paged inbox and newest-first message pages', async () => {
  ...
  assert.match(mobileStyles, /\.figma-friends-screen\.is-messages\.is-conversation-open \.figma-friends-message-layout\s*\{[\s\S]*grid-template-rows: minmax\(0, 1fr\) !important;/s)
  assert.match(mobileStyles, /\.figma-friends-screen\.is-messages\.is-conversation-open \.figma-friends-chat\s*\{[\s\S]*position: relative !important;/s)
  assert.match(densityStyles, /\.figma-friends-screen\.is-messages:not\(\.is-conversation-open\) \.figma-friends-conversations/)
