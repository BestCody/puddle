...
test('active messaging uses cursor-paged inbox and newest-first message pages', async () => {
  ...
  assert.match(mobileStyles, /:has\(\.figma-friends-chat > header\) \.figma-friends-message-layout\s*\{[\s\S]*grid-template-rows: minmax\(0, 1fr\) !important;/s)
  assert.match(mobileStyles, /:has\(\.figma-friends-chat > header\) \.figma-friends-chat\s*\{[\s\S]*position: relative !important;/s)
  assert.doesNotMatch(mobileStyles, /:has\(\.figma-friends-chat > header\) \.figma-friends-chat\s*\{[\s\S]*position: fixed !important;/s)
