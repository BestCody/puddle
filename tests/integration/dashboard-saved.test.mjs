...
test('rebuilt Figma dashboard shell keeps the authored expanded and concise sidebar states accessible', async () => {
  ...
  assert.match(finalStyles, /\.figma-dashboard-nav-item\s*\{[\s\S]*width:\s*241px !important[\s\S]*height:\s*50px !important/)
  assert.match(finalStyles, /\.figma-dashboard-nav,[\s\S]*top:\s*50% !important[\s\S]*gap:\s*2px !important[\s\S]*transform:\s*translateY\(-50%\) !important/)
  assert.match(finalStyles, /\.figma-dashboard-sidebar-logo,[\s\S]*top:\s*calc\(50% - 223px\) !important/)
