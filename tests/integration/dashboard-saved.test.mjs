...
test('rebuilt Figma dashboard shell keeps the authored expanded and concise sidebar states accessible', async () => {
  ...
  assert.match(finalStyles, /\.figma-dashboard-nav-item\s*\{[\s\S]*width:\s*217px !important[\s\S]*height:\s*50px !important/)
  assert.match(finalStyles, /\.figma-dashboard-nav,[\s\S]*left:\s*16px !important[\s\S]*width:\s*217px !important[\s\S]*gap:\s*2px !important[\s\S]*transform:\s*translateY\(-50%\) !important/)
  assert.match(finalStyles, /\.figma-dashboard-sidebar-logo,[\s\S]*top:\s*calc\(50% - 223px\) !important[\s\S]*left:\s*31px !important/)
