...
test('Saved Place Open preserves Figma 38:223 relationships through scoped structural layout', async () => {
  ...
  assert.match(page, /const primaryFolders = folders\.slice\(0, 2\)/)
  assert.match(page, /const overflowFolders = folders\.slice\(2\)/)
  assert.match(page, /<details className=\{styles\.moreCategories\}>/)
  assert.match(page, /<summary aria-label="More saved categories">\+<\/summary>/)
  assert.match(page, /overflowFolders\.map\(\(folder\) => <Link href=\{`\/plans\?tab=saved&category=/)
