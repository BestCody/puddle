...
test('global discovery failures never fail over to Supabase/Postgres', async () => {
  ...
  assert.doesNotMatch(discoverySource, /global-location-stale-cache|global-location-degraded|emptyDegradedFeed|CIRCUIT_COOLDOWN_MS|STALE_TTL_MS/)
  assert.ok(discoverySource.includes('No fallbacks'))
  assert.ok(discoverySource.includes('serving failures throw'))
})
