...
test('Hot cached routes load the B2 search graph only when catalogue data is needed', async () => {
  ...
  for (const source of [discovery, map, socialFeed, plans, publicLocation, mapData]) {
  ...
  assert.match(publicContent, /getCachedPublicLocation/)
  assert.doesNotMatch(publicContent, /global-location-search/)
})
...
test('Social feed uses one indexed post-page read and a shell-first API render', async () => {
  ...
  assert.match(feed, /social-feed-location-hydration-v3/)
  assert.match(feed, /cachedSocialLocations\(JSON\.stringify\(uniqueIds\)\)/)
  assert.match(feed, /resolvedCount: rows\.length/)
  ...
  assert.match(client, /data-testid="feed-load-sentinel"/)
