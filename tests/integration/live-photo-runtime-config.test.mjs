...
test('browser Google fallback stays budget-gated and never persists client identity', async () => {
  ...
  const clientLookup = await read('lib/app/google-place-client.js')
  ...
  assert.match(fallback, /findGoogleUiKitPlace/)
  assert.match(fallback, /gmp-place-details-place-request/)
  assert.match(clientLookup, /gmp-place-search/)
  assert.match(clientLookup, /gmp-place-text-search-request/)
