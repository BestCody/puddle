...
test('rebuilt Figma swipe card keeps drag, dock, keyboard, durable persistence, and stacked deck motion', () => {
  ...
  assert.match(cardSource, /const exitDistance = horizontalSwipe \? Math\.max\(window\.innerWidth \+ 240, 960\) : 0/)
  assert.match(cardSource, /setDragX\(action === 'pass' \? -exitDistance : action === 'save' \? exitDistance : 0\)/)
  assert.match(cardSource, /onExitChange\?\.\(true\)/)
  assert.match(cardSource, /onExitChange\?\.\(false\)/)
  assert.match(cardSource, /preview = false/)
  assert.match(cardSource, /is-deck-preview/)
  assert.match(cardSource, /is-deck-front/)
  ...
  assert.match(workspaceSource, /const nextCard = feed\.items\[index \+ 1\] \|\| null/)
  assert.match(workspaceSource, /FigmaSwipeCard item=\{nextCard\}/)
  assert.match(workspaceSource, /item=\{nextCard\} onChoice=\{\(\) => \{\}\} busy preview/)
  assert.match(workspaceSource, /item=\{current\} onChoice=\{persistChoice\}/)
  assert.match(workspaceSource, /onExitChange=\{setCardExiting\}/)
  ...
  assert.match(motionCss, /\.figma-swipe-card-stage\.is-advancing \.figma-swipe-card-layer\.is-back/)
  assert.match(motionCss, /transform:translateY\(0\) scale\(1\)/)
})
