...
test('authenticated desktop pages follow the Puddle Official Figma geometry', async ({ page }, testInfo) => {
  ...
  const sidebarLogo = sidebar.locator('.minimal-sidebar-logo img')
  await expect(sidebarLogo).toHaveAttribute('src', '/puddle-mark-outline.svg')
  await expectBox(sidebarLogo, { x: 34, y: 184, width: 56, height: 56 }, 'Sidebar Figma logo', 2)
  await expectBox(sidebar.locator('a[href="/map"] .figma-feed-icon'), { width: 28, height: 28 }, 'Feed compass glyph', 1)
  await expectBox(sidebar.locator('a[href="/matches"] .figma-friends-icon'), { width: 25, height: 25 }, 'Friends bubble glyph', 1)
  ...
  await expectBox(page.locator('.minimal-swipe-card'), { x: 580, y: 100, width: 400, height: 560 }, 'Swipe card', 2)
  await expectBox(page.locator('.minimal-swipe-actions'), { x: 625, y: 666, width: 310 }, 'Swipe controls', 3)
  await expectBox(page.locator('.minimal-swipe-details-button'), { x: 920, y: 600, width: 41, height: 40 }, 'Swipe details button', 3)
  ...
  await expectBox(passCards.first(), { x: 403, y: 222, width: 345, height: 455 }, 'Free card', 4)
  await expectBox(passCards.nth(1), { x: 770, y: 220, width: 345, height: 457 }, 'Pass card', 4)
  ...
  await expectBox(page.locator('.minimal-profile-card::before'), {}, 'Profile cover', 4).catch(() => {})
  await expectBox(page.locator('.minimal-profile-card > a'), { x: 1059, y: 43, width: 143, height: 50 }, 'Profile Edit button', 3)
  await expectBox(page.locator('.minimal-profile-avatar'), { x: 697, y: 241, width: 133, height: 133 }, 'Profile avatar', 4)
  const profileActions = page.locator('.figma-profile-actions a')
  await expectBox(profileActions.nth(0), { x: 531, y: 573, width: 218, height: 50 }, 'Profile Follow button', 4)
  await expectBox(profileActions.nth(1), { x: 759, y: 573, width: 218, height: 50 }, 'Profile Message button', 4)
  await expectBox(page.locator('.minimal-profile-settings > :nth-child(1)'), { x: 378, y: 660, width: 369, height: 378 }, 'Puddles panel', 4)
  await expectBox(page.locator('.figma-profile-mini-puddle'), { x: 394, y: 727, width: 338, height: 291 }, 'Profile mini Puddle', 5)
