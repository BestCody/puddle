...
test('authenticated desktop dashboard keeps navigation and core product behavior usable', async ({ page }, testInfo) => {
  ...
  await expect(signOut).toHaveCSS('color', 'rgb(180, 35, 24)')
  ...
  await expect(page.locator('.puddle-feed-search-overlay .puddle-discover-search-backdrop')).toHaveCSS('backdrop-filter', 'none')
  await page.keyboard.press('Escape')
  await expect(feedSearch).toHaveAttribute('aria-expanded', 'false')
  ...
  const savedSearchInput = savedSearch.getByRole('searchbox', { name: 'Search saved puddles' })
  await expect(savedSearchInput).toHaveAttribute('placeholder', 'Search a saved puddle...')
  ...
  await savedSearchInput.fill('park')
  await expect(page).toHaveURL(/\/plans\?tab=saved&q=park/)
  await savedSearchInput.fill('')
