...
test('public Swipe phone keeps the Figma screen identity and real draggable mechanics', async ({ page }) => {
  ...
  await expect(screen.locator('.figma-swipe-card-photo img').first()).toBeVisible()
  expect(await screen.locator('.figma-swipe-card-photo img').first().evaluate((image) => image.naturalWidth)).toBeGreaterThan(0)

  const card = screen.locator('.figma-swipe-card[data-card-role="active"]')
