...
test('public Swipe phone keeps the Figma screen identity and real draggable mechanics', async ({ page }) => {
  ...
  await expect(screen.locator('.landing-demo-swipe-photo img')).toBeVisible()
  expect(await screen.locator('.landing-demo-swipe-photo img').evaluate((image) => image.naturalWidth)).toBeGreaterThan(0)
...
test('Saved, Feed, and Profile phone demos expose their corresponding Figma interactions', async ({ page }) => {
  ...
  await expect(feed.locator('.landing-demo-feed-pictures img').first()).toBeVisible()
  const feedImageWidths = await feed.locator('img').evaluateAll((images) => images.map((image) => image.naturalWidth))
  expect(feedImageWidths.filter((width) => width > 0).length).toBeGreaterThanOrEqual(5)
