...
try {
  ...
  assert(await jump.isVisible() && jumpBox && jumpBox.y > hero.y + hero.height * .22 && jumpBox.y < hero.y + hero.height * .32, 'Figma Jump In action must be visible in the initial mobile hero')
  assert(await jump.getAttribute('aria-hidden') !== 'true', 'visible Figma Jump In action must not be hidden from accessibility')
