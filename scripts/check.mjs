...
const required = [
  'package.json','next.config.mjs','proxy.js','.env.example','index.html','styles.css','app.js','landing-demo.js',
  'public/landing.html','public/styles.css','public/app.js','public/landing-demo.js','app/layout.js','app/auth.css',
...
for (const path of ['next.config.mjs','proxy.js','lib/supabase/env.js','lib/supabase/server.js','lib/supabase/proxy.js','lib/auth/redirect.js','scripts/check.mjs','app.js','landing-demo.js']) {
  execFileSync(process.execPath, ['--check', join(root, path)], { stdio: 'pipe' })
}
for (const [source, served] of [['index.html','public/landing.html'],['styles.css','public/styles.css'],['app.js','public/app.js'],['landing-demo.js','public/landing-demo.js']]) {
...
const landingAuth=await readFile(join(root,'app.js'),'utf8')
for (const target of ['/signup','/signin','data-open-app','data-open-modal']) if(!landingAuth.includes(target)) throw new Error(`Landing authentication wiring is missing ${target}`)
