...
try {
  for (const [width, height] of cases) {
    ...
    const capturedHeight = await page.evaluate(() => document.documentElement.scrollHeight)
    console.log(`captured complete page ${width}x${height}: ${width}x${capturedHeight}px -> ${output}`)
    await page.close()
  }
} finally {
  await browser.close()
