...
const migration = (await Promise.all(['0014_ai_creation_and_embeddings.sql','0015_hybrid_recommendation_foundation.sql','0016_hybrid_recommendation_runtime.sql','0017_stage8_authorization.sql','0018_stage8_hardening.sql'].map((name) => readFile(join(root, 'supabase/migrations', name), 'utf8')))).join('\n')
