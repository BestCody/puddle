-- One-time registration path for B2 global-photo metadata created before the
-- uniqueness registry existed. The caller verifies the immutable B2 object and
-- supplies both fingerprints; this RPC atomically installs a live claim or
-- reports the conflicting owner so the stale metadata row can be excluded.
...
begin
  ...
  -- If this location is already represented by exactly these bytes, the B2
  -- metadata is already consistent with the registry and needs no exclusion.
  select g.content_sha256 into v_existing_content
  from public.global_photo_claims g
  where g.location_id=p_location_id and g.status='live'
  limit 1;
  if found then
  ...
  from public.global_photo_claims g
  where g.content_sha256=v_content_hash and g.status='live'
  limit 1;
  if found then
  ...
  select g.location_id into v_conflict
  from public.global_photo_claims g
  where g.provider_asset_hash=v_provider_hash and g.status='live'
  limit 1;
  if found then
