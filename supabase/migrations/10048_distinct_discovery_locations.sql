-- Keep the personal discovery deck distinct across reloads, filter changes, and refills.
-- A location counts as seen after a completed swipe-like action. Opening details alone
-- does not remove the place from future discovery.
...
as $$
  with latest_swipe as (
    ...
    select dismissal.location_id
    from public.static_catalogue_actions dismissal
    where dismissal.user_id=auth.uid()
