...
-- Creating a location is a Pass entitlement. Existing creators can continue to edit
-- their already-created locations through the existing update policy.
...
-- Browser notification alerts rely on authenticated realtime INSERT events.
do $$
begin
  if not exists (
    select 1
