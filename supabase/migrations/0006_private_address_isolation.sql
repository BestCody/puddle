-- Stage 2 hardening: isolate exact private addresses from publicly readable listing rows.
-- Apply after 0005_content_creation_and_publication.sql.
...
create table if not exists public.location_private_details (
...
alter table public.events add column if not exists has_private_address boolean not null default false;
alter table public.locations add column if not exists has_private_address boolean not null default false;

insert into public.event_private_details(event_id,exact_address,updated_by,updated_at)
select id,private_address,created_by,coalesce(updated_at,now())
from public.events
where nullif(trim(coalesce(private_address,'')),'') is not null
on conflict(event_id) do update set exact_address=excluded.exact_address,updated_by=excluded.updated_by,updated_at=excluded.updated_at;

insert into public.location_private_details(location_id,exact_address,updated_by,updated_at)
select id,private_address,created_by,coalesce(updated_at,now())
from public.locations
where nullif(trim(coalesce(private_address,'')),'') is not null
on conflict(location_id) do update set exact_address=excluded.exact_address,updated_by=excluded.updated_by,updated_at=excluded.updated_at;

update public.events set has_private_address=true where exists(select 1 from public.event_private_details d where d.event_id=events.id and nullif(trim(coalesce(d.exact_address,'')),'') is not null);
update public.locations set has_private_address=true where exists(select 1 from public.location_private_details d where d.location_id=locations.id and nullif(trim(coalesce(d.exact_address,'')),'') is not null);

update public.event_revisions set snapshot=snapshot-'private_address' where snapshot ? 'private_address';
update public.location_revisions set snapshot=snapshot-'private_address' where snapshot ? 'private_address';

alter table public.events drop column if exists private_address;
alter table public.locations drop column if exists private_address;

alter table public.event_private_details enable row level security;
alter table public.location_private_details enable row level security;

create policy "event managers manage private details" on public.event_private_details for all
using (public.can_manage_event(event_id) or public.is_admin())
with check (public.can_manage_event(event_id) or public.is_admin());

create policy "location managers manage private details" on public.location_private_details for all
using (public.can_manage_location(location_id) or public.is_admin())
with check (public.can_manage_location(location_id) or public.is_admin());

grant select,insert,update,delete on public.event_private_details,public.location_private_details to authenticated;
...
begin
  ...
  if new.status<>'draft' and new.event_format in ('in_person','hybrid','private') and new.location_id is null and nullif(trim(coalesce(new.address_public,'')),'') is null and not new.has_private_address then raise exception 'In-person events require a location or address'; end if;
  return new;
end;
$$;
...
begin
  ...
  if record_location.name is null or record_location.city is null or (record_location.address_public is null and not record_location.has_private_address) then raise exception 'Complete the required location details'; end if;
