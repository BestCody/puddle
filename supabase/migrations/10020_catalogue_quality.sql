...
-- Backfill the current Canadian catalogue immediately; the queued refresh below will
-- fill richer fields from source records and apply category mapping v2 idempotently.
update public.locations set country='Canada' where country_code='CA' and country is null;
with subdivisions(code,name) as (values
...
update public.locations location
set region_code=subdivisions.code,region=subdivisions.name
from subdivisions
where location.country_code='CA' and upper(coalesce(location.region,''))=subdivisions.code;

update public.locations location
set photo_enrichment_status=case when exists(
  select 1 from public.location_photo_sources photo
  where photo.location_id=location.id and photo.status='approved' and not coalesce(photo.is_ai_generated,false)
) then 'matched' else 'pending' end,
photo_retry_after=null
where location.source='import';

update public.catalogue_sync_regions
set status='queued',requested_at=now(),synced_at=null,release_id=null,
    error_message='Requeued for catalogue geography, category, relationship, and photo-quality backfill',
    updated_at=now()
where source='overture' and status in ('ready','empty');
