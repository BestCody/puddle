-- Stage 2 schema and authorization assertions. Run after applying migrations through 0005.
do $$
declare missing text;
begin
  select string_agg(name, ', ') into missing
  from (values ('event_occurrences'),('event_revisions'),('location_revisions'),('location_claims')) expected(name)
  ...
  select string_agg(name, ', ') into missing
  from (values
    ('events.event_format'),('events.visibility'),('events.recurrence_rule'),('events.publish_at'),('events.attendee_questions'),('events.accessibility'),('events.contact_links'),
    ('locations.private_address'),('locations.visibility'),('locations.tags'),('locations.contact_links'),('locations.claimed_by_host_id'),('host_profiles.contact_links')
  ) expected(name)
  where not exists (
  ...
  from (values
    ('published event occurrences read'),('event managers manage occurrences'),('event managers read revisions'),('location managers read revisions'),
