...
export async function generateMetadata({ params }) {
  ...
  return { title:location.name,description:location.summary||location.description,alternates:{canonical:`/places/${location.slug}`},openGraph:{type:'website',title:location.name,description:location.summary||location.description,url:`/places/${location.slug}`,images:[{url:location.cover_url||'/og-puddle.svg',width:1200,height:630,alt:location.name}]}}
}
export default async function PlacePage({ params }) {
  ...
  return <><script type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify(structured).replace(/</g,'\u003c')}}/><PublicLocationView {...result}/><ListingSocial kind="location" content={result.location} returnTo={returnTo}/></>
}
