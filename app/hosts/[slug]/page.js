...
export async function generateMetadata({ params }) {
  ...
  return {
    title: result.host.name,
    description: result.host.description,
    alternates: { canonical: `/hosts/${result.host.slug}` },
    openGraph: {
      title: result.host.name,
      description: result.host.description,
      url: `/hosts/${result.host.slug}`,
      images: [{ url: '/og-puddle.svg', width: 1200, height: 630, alt: result.host.name }]
    }
  }
}
