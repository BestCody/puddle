...
export async function generateMetadata({ params }) {
  ...
  return {
    ...
    openGraph: {
      ...
      images: [{ url: event.cover_url || '/events/neon-night.svg', width: 1200, height: 630, alt: event.title }]
    }
  }
}
...
export default async function EventPage({ params }) {
  ...
  return <>
    <script nonce={nonce} type="application/ld+json" dangerouslySetInnerHTML={{ __html: serializeStructuredData(structured) }} />
    <PublicEventView {...result} />
    <TicketPurchasePanel eventId={result.event.id} eventSlug={result.event.slug} tiers={tiers} />
    <TemporaryLocationSharing contextKind="event" contextId={result.event.id} contextLabel={result.event.title} />
    <ListingSocial kind="event" content={result.event} returnTo={returnTo} />
  </>
}
