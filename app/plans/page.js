...
function ListingCard({ item }) {
  const title = item.title || item.events?.title || item.locations?.name || 'Untitled'
  const href = item.href || (item.events?.slug ? `/events/${item.events.slug}` : item.locations?.slug ? `/places/${item.locations.slug}` : '/plans')
  return <article className="plan-content-card"><span>{item.kind || (item.events ? 'event' : item.locations ? 'place' : 'plan')}</span><h2><Link href={href}>{title}</Link></h2><p>{item.summary || item.events?.summary || item.locations?.summary || item.status || 'Plan details'}</p>{item.startsAt || item.events?.starts_at ? <small>{new Date(item.startsAt || item.events.starts_at).toLocaleString('en-CA', { month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' })}</small> : null}</article>
}

function GoingCard({ item }) {
...
function SharedCard({ plan, userId }) {
  const invited = plan.membership_status === 'invited'
...
export default async function PlansPage({ searchParams }) {
  ...
  const active = tabs.some(([value]) => value === params?.tab) ? params.tab : 'saved_events'
  return renderProductPage(async (session) => {
    const snapshot = await getPlansSnapshot(session)
    const items = active === 'saved_events' ? snapshot.savedEvents
      : active === 'saved_places' ? snapshot.savedPlaces
      : active === 'interested' ? snapshot.interested
      : active === 'going' ? snapshot.going
      : active === 'tickets' ? snapshot.tickets
      : active === 'hosting' ? snapshot.hosting
      : active === 'shared' ? snapshot.sharedPlans
      : snapshot.past
    return (
      <>
        <section className="page-heading-row"><div><span className="section-pill section-pill-yellow">Your orbit</span><h1 className="product-title">Plans, not maybes.</h1><p>Saved events, saved places, attendance, hosting, visits, and collaborative itineraries live together.</p></div><Link className="splash-button splash-button-pink" href="/discover">Find something</Link></section>
