...
function confirmationFailure(url, code = 'confirmation_failed') {
  ...
  const target = new URL('/landing.html', url)
...
export async function GET(request) {
  ...
  if (!isSupabaseConfigured()) return NextResponse.redirect(new URL('/landing.html?error=Accounts+are+temporarily+unavailable.+Please+try+again+later.', url))
