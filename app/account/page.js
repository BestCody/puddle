...
import { deleteAccount, revokeOtherSessions, updatePassword, updateProfile } from '@/app/auth/actions'
  ...
  return (
    <ProductShell user={user} profile={profile}>
      ...
      <div className="settings-grid">
        <form className="settings-card" action={updateProfile}>
          <h2>Public profile</h2>
          <label className="field">Display name<input name="display_name" defaultValue={profile?.display_name || ''} required /></label>
          <label className="field">Username<input name="username" defaultValue={profile?.username || ''} required /></label>
          <label className="field">City<input name="city" defaultValue={profile?.city || ''} /></label>
          <label className="field">Bio<textarea name="bio" defaultValue={profile?.bio || ''} maxLength="500" /></label>
          <label className="field">Search radius<input name="search_radius_km" type="number" min="1" max="100" defaultValue={profile?.search_radius_km || 10} /></label>
          <label className="field">Visibility<select name="profile_visibility" defaultValue={profile?.profile_visibility || 'friends'}><option value="hidden">Hidden</option><option value="friends">Friends</option><option value="mutuals">Mutuals</option><option value="attendees">Event attendees</option><option value="public">Public</option></select></label>
