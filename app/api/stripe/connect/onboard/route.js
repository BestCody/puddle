...
export async function POST() {
  ...
  try {
    ...
    if (!record?.stripe_account_id) {
      ...
      const inserted = await admin.from('stripe_connected_accounts').upsert({
        profile_id: user.id,
      ...
      }, { onConflict: 'profile_id' }).select('profile_id').single()
      if (inserted.error) throw inserted.error
    } else {
      account = await retrieveAccount(record.stripe_account_id)
    }
    ...
    const link = await createAccountLink({
      ...
      refreshUrl: `${base}/settings/payouts?stripe=refresh`,
      returnUrl: `${base}/settings/payouts?stripe=returned`
    })
