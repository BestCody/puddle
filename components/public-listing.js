...
function PublicHeader() { return <header className="public-header"><PuddleLogo /><nav><Link href="/">Home</Link><Link href="/signin">Sign in</Link><Link className="public-primary-link" href="/signup">Register</Link></nav></header> }
function HostChip({ host }) {
  return host ? <Link className="public-host-chip" href={`/hosts/${host.slug}`}>{host.logo_url ? <img src={host.logo_url} alt=""/> : <span>{host.name.slice(0, 1).toUpperCase()}</span>}<div><strong>{host.name}</strong><small>{host.verification_status === 'verified' ? '✓ Verified host' : human(host.kind)}</small></div></Link> : <div className="public-host-chip"><span>P</span><div><strong>Puddle member</strong><small>Personal host</small></div></div>
}
function Gallery({ items = [], title }) {
  if (!items.length) return null
  return <section className="public-section"><div className="public-section-heading"><span className="section-pill section-pill-yellow">Gallery</span><h2>{title}</h2></div><div className="public-gallery">{items.map((item)=><figure key={item.id}><img src={item.url} alt={item.caption || ''}/>{item.caption?<figcaption>{item.caption}</figcaption>:null}</figure>)}</div></section>
}
function SimilarGrid({ items = [] }) {
  ...
  return <section className="public-section"><div className="public-section-heading"><span className="section-pill section-pill-mint">Keep exploring</span><h2>Similar splashes.</h2></div><div className="public-similar-grid">{items.map((item) => {
    ...
    return <Link href={href} className="public-similar-card" key={`${event ? 'event' : 'place'}-${item.id || item.slug}`} style={item.cover_url?{backgroundImage:`linear-gradient(rgba(255,255,255,.9),rgba(255,255,255,.96)),url(${item.cover_url})`}:undefined}><span>{event ? 'EVENT' : 'PLACE'}</span><strong>{title}</strong><p>{item.summary || (event ? 'A plan worth checking out.' : 'A place worth saving.')}</p><em>{event ? (item.starts_at ? new Date(item.starts_at).toLocaleDateString('en-CA', { month: 'short', day: 'numeric' }) : 'See details') : item.city || 'See details'} →</em></Link>
  })}</div></section>
}
...
export function PublicEventView({ event, similar = [], preview = false }) {
  ...
  return <div className="public-page"><PublicHeader /><main className="public-wrap">
    ...
    <section className="public-listing-hero event-hero"><div className="public-hero-art" style={event.cover_url?{backgroundImage:`linear-gradient(180deg,rgba(23,17,20,.04),rgba(23,17,20,.64)),url(${event.cover_url})`}:undefined}><span>{String(event.category || 'event').toUpperCase()}</span>{!event.cover_url?<strong>✦</strong>:null}<small>{event.status || 'published'}</small></div><div className="public-hero-copy"><span className="section-pill section-pill-yellow">Event</span><h1>{event.title}</h1><p>{event.summary || 'A plan worth leaving the house for.'}</p><div className="public-meta-row"><span>{new Date(event.starts_at).toLocaleString('en-CA', { weekday: 'long', month: 'long', day: 'numeric', hour: 'numeric', minute: '2-digit', timeZone: event.timezone || undefined })}</span><span>{locationLabel}</span><span>{money(event.price_from_cents, event.currency)}</span>{event.min_age ? <span>{event.min_age}+</span> : <span>All ages</span>}</div><div className="public-action-row"><ContentActionButton contentKind="event" contentId={event.id}>Save this event</ContentActionButton><Link className="public-secondary-cta" href={`/events/${event.slug}/join`}>Join event</Link><Link className="public-secondary-cta" href={`/events/${event.slug}/calendar`}>Add to calendar</Link></div></div></section>
    ...
    <Gallery items={event.gallery} title="See the atmosphere." />
