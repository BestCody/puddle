  ...
  return <div className="date-details-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget && !busy) onCancel() }}>
    <section className="date-choice-sheet" role="dialog" aria-modal="true" aria-labelledby="date-choice-title">
      ...
      <div className="date-choice-actions">
        ...
        <button className={perfect ? 'is-perfect' : ''} type="button" onClick={() => onSubmit(note)} disabled={busy}>{busy ? 'Saving…' : perfect ? 'Make it my Perfect Pick' : 'Save choice'}</button>
      </div>
    </section>
  </div>
}
...
function MatchCelebration({ match, onClose, onPlan }) {
  ...
  return <div className="date-match-celebration" role="dialog" aria-modal="true" aria-labelledby="date-match-title">
    ...
    <section>
      ...
      <div>
        <button type="button" onClick={() => onPlan(match)}>Plan this date</button>
        <Link href={match.item.href}>View details</Link>
        <button type="button" onClick={onClose}>Keep swiping</button>
      </div>
    </section>
  </div>
}
...
function ScheduleModal({ target, onClose, onSave, busy }) {
  ...
  return <div className="date-details-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget && !busy) onClose() }}>
    <section className="date-choice-sheet" role="dialog" aria-modal="true" aria-labelledby="date-plan-title">
      <span className="section-pill">Make it real</span>
      <h2 id="date-plan-title">Plan {target.item.title}</h2>
      <p>Pick a time for the date. Both people will see it in this DateMatch room.</p>
...
function FeedbackPrompt({ match, item, onFeedback, busy }) {
...
function DateMatchSummary({ items, matches, partnerJoined, shareUrl, onPlan, onFeedback, busy }) {
  ...
  return <section className="date-deck-summary">
    ...
    <div className="date-summary-actions"><button type="button" onClick={() => shareDateMatch(shareUrl)}>Send this deck to my date</button><Link href="/discover">Start another deck</Link></div>
    {feedbackTarget && feedbackItem ? <FeedbackPrompt match={feedbackTarget} item={feedbackItem} busy={busy} onFeedback={(happened, rating) => onFeedback(feedbackTarget, happened, rating)} /> : null}
  </section>
}
...
export function DateMatchWorkspace({ initialSnapshot, googleMapsBrowserKey = '' }) {
  ...
  async function persistSwipe(choice, item, note = '') {
    setBusy(true)
    const response = await csrfFetch('/api/date-match/action', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ action: 'swipe', deckId: initialSnapshot.deck.id, locationId: item.content_id, choice, note })
    })
    const payload = await response.json().catch(() => ({}))
    if (!response.ok) {
      setMessage(payload.error || 'That choice could not be saved.')
      setBusy(false)
      return
    }
  ...
  function requestChoice(choice, item) {
    if (choice === 'save' || choice === 'perfect') setPendingChoice({ choice, item })
    else persistSwipe('pass', item)
  }

  async function shareRoom() {
    try { setMessage(await shareDateMatch(window.location.href)) } catch (error) { if (error?.name !== 'AbortError') setMessage('The invitation could not be shared from this browser.') }
  }

  async function schedule(plannedFor) {
    if (!scheduleTarget) return
    setBusy(true)
    const match = scheduleTarget.match || scheduleTarget
  ...
  return <div className="date-swipe-workspace date-match-workspace">
    <div className="date-swipe-toolbar">
      <button className="date-filter-toggle date-swipe-together" type="button" onClick={shareRoom}>↗ Share invitation</button>
      <span>{Math.min(Object.keys(choices).length + 1, items.length)} of {items.length} date ideas</span>
      <Link href="/plans?tab=shared">Open shared plans →</Link>
    </div>
    {!initialSnapshot.deck.partnerJoined ? <div className="date-match-waiting"><strong>Your date has not joined yet.</strong><span>You can swipe now; your choices remain hidden until they choose the same place.</span><button type="button" onClick={shareRoom}>Send invitation</button></div> : null}
    ...
    <div className={`date-deck-stage ${current && index < items.length - 1 ? 'has-next-card' : ''}`}>
      {current ? <DateLocationCard key={current.content_id} item={current} onChoice={requestChoice} onMessage={setMessage} busy={busy} googleMapsBrowserKey={googleMapsBrowserKey} allowPerfect puddlePick={current.is_puddle_pick} partnerNote={current.partner_note} /> : <DateMatchSummary items={items} matches={matches} partnerJoined={initialSnapshot.deck.partnerJoined} shareUrl={shareUrl} onPlan={setScheduleTarget} onFeedback={feedback} busy={busy} />}
    </div>
