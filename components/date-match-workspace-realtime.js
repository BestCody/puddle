...
export function DateMatchWorkspaceRealtime({ initialSnapshot, googleMapsBrowserKey = '' }) {
  ...
  useEffect(() => {
    ...
    async function refreshRoom() {
      ...
      activeRefresh = fetch(`/api/date-match/${encodeURIComponent(initialSnapshot.token)}`, { cache: 'no-store' })
        .then(async (response) => response.ok ? response.json().catch(() => null) : null)
        .then((snapshot) => { if (snapshot && !cancelled) applySnapshot(snapshot) })
    ...
    const fallback = window.setInterval(() => refreshRoom().catch(() => {}), 90_000)
    ...
    try {
      ...
      channel = supabase
        .channel(`date-match-room-${deck.id}-${crypto.randomUUID()}`)
        .on('postgres_changes', {
          event: '*', schema: 'public', table: 'date_match_room_versions', filter: `deck_id=eq.${deck.id}`
        }, scheduleRefresh)
        .subscribe()
    } catch {
      // The 90-second fallback remains available if Realtime cannot initialize.
    }
    ...
    return () => {
      ...
      if (channel) createClient().removeChannel(channel).catch(() => {})
    }
  }, [initialSnapshot.token, deck.id, group])
