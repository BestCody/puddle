...
function DiscoveryCard({ item, onAction, compact = false }) {
  return (
    <article className={`real-discovery-card ${compact ? 'is-compact' : ''}`}>
      <div className="real-discovery-art" style={{ backgroundImage: item.cover_url ? `linear-gradient(180deg,transparent,rgba(20,14,18,.55)),url(${item.cover_url})` : undefined }}>
        ...
        <em>{Math.round(item.score)} pts</em>
      </div>
      <div className="real-discovery-copy">
        ...
        <h2>{item.title}</h2>
        <p>{item.summary || 'Worth a closer look.'}</p>
...
function MapCanvas({ items, onSelect, selectedId }) {
  ...
  return (
    <div className="puddle-map" role="img" aria-label="Map of nearby events and places">
...
export function DiscoveryWorkspace({ initialFeed, defaultMode = 'deck' }) {
  ...
  async function refresh(nextFilters = filters) {
    ...
    setLoading(false)
    if (!response.ok) {
      setMessage(result.error || 'Discovery could not refresh.')
      return
    }
    setFeed(result)
  ...
  async function useLocation() {
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        const next = { ...filters, latitude: position.coords.latitude, longitude: position.coords.longitude }
        setFilters(next)
        refresh(next)
      },
  ...
  async function act(action, item) {
    setMessage(`${action === 'dismissed' ? 'Passed' : action === 'saved' ? 'Saved' : 'Interested'} · ${item.title}`)
    const response = await fetch('/api/discovery/action', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ action, contentKind: item.content_kind, contentId: item.content_id, requestId: feed.requestId })
    })
    if (!response.ok) {
      const result = await response.json().catch(() => ({}))
    ...
    if (mode === 'deck') setIndex((current) => Math.min(current + 1, Math.max(feed.items.length - 1, 0)))
  }

  async function undo() {
    const item = feed.items[Math.max(0, index - 1)] || feed.items[index]
    if (!item) return
    await fetch('/api/discovery/action', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ action: 'undo', contentKind: item.content_kind, contentId: item.content_id, requestId: feed.requestId })
    })
    setIndex((current) => Math.max(0, current - 1))
    setMessage('Last discovery choice undone.')
  }
  ...
  return (
    <div className="discovery-workspace">
      ...
      <div className="discovery-mode-row">
        <div className="mode-tabs">{['deck','list','map'].map((value) => <button className={mode === value ? 'is-active' : ''} type="button" onClick={() => setMode(value)} key={value}>{value}</button>)}</div>
        <span>{feed.items.length} result{feed.items.length === 1 ? '' : 's'} · {feed.rankingVersion}</span>
      </div>

      {message ? <p className="discovery-message">{message}</p> : null}
      {feed.fallback ? <div className="demo-data-note">A few starter picks are shown while your area fills up.</div> : null}
