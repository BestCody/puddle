...
export function b2PublicUrl(baseUrl, key) {
  const base = required(baseUrl, 'B2 public base URL').replace(/\/+$/, '')
  const url = new URL(base)
  if (url.protocol !== 'https:') throw new Error('B2 public base URL must use HTTPS.')
...
export function b2ConfigFromEnv(prefix = 'B2_MEDIA', env = process.env) {
  ...
  return {
    keyId: text(env[`${normalized}_KEY_ID`] || env[`${normalized}_APPLICATION_KEY_ID`]),
    applicationKey: text(env[`${normalized}_APPLICATION_KEY`]),
    bucketId: text(env[`${normalized}_BUCKET_ID`]),
    bucketName: text(env[`${normalized}_BUCKET_NAME`]),
    publicBaseUrl: text(env[`${normalized}_PUBLIC_BASE_URL`])
  }
}
...
export async function createB2BucketClient(config = {}, { fetchFn = fetch } = {}) {
  ...
  function publicUrl(key) {
    if (config.publicBaseUrl) return b2PublicUrl(config.publicBaseUrl, key)
    if (!bucket.name) throw new Error('B2 public base URL or bucket name is required to build a delivery URL.')
    return `${auth.downloadUrl}/file/${encodeURIComponent(bucket.name)}/${String(key).split('/').map(encodeURIComponent).join('/')}`
  }
    ...
    async function uploadBody({ key, bodyFactory, byteSize, sha1, contentType = 'b2/x-auto', metadata = {} }) {
      ...
      for (let attempt = 0; attempt < 5; attempt += 1) {
        ...
        try {
          ...
          return { ...result, key: safeKey, publicUrl: publicUrl(safeKey) }
        } catch (error) {
          uploadSession = null
          if (!RETRYABLE.has(Number(error.status)) || attempt === 4) throw error
          if (error.status === 401) await refreshAuthorization()
