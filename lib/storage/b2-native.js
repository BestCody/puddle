...
export async function authorizeB2({ keyId, applicationKey, env = process.env, fetchFn = fetch } = {}) {
  ...
  const authorizeUrl = text(env?.B2_AUTHORIZE_ENDPOINT) || DEFAULT_AUTHORIZE_URL
  const response = await fetchFn(authorizeUrl, {
...
export function b2ConfigFromEnv(prefix = 'B2_MEDIA', env = process.env) {
  ...
  return {
    keyId: scoped(env, [`${normalized}_APPLICATION_KEY_ID`]),
    applicationKey: scoped(env, [`${normalized}_APPLICATION_KEY`]),
    bucketId: scoped(env, [`${normalized}_BUCKET_ID`]),
    bucketName: scoped(env, [`${normalized}_BUCKET_NAME`])
  }
}
