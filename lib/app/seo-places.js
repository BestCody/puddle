...
export function describeHubPlaces(places, { market, category } = {}) {
  ...
  const named = listSentence(all.slice(0, 3).map((place) => place.name))
  const singular = category ? category.singular : 'place'
  const plural = category ? category.label.toLowerCase() : 'places'
  const noun = all.length === 1 ? singular : plural
  const where = market ? ` around ${market.name}` : ''
  const rest = all.length > 3 ? `, plus ${all.length - 3} more` : ''
  const withPhotos = all.filter((place) => place.coverUrl).length
  const photoNote = withPhotos >= 3 ? ` ${withPhotos} of them have photos.` : ''
  return `Includes ${named}${rest}. Puddle tracks ${all.length} ${noun}${where}.${photoNote}`
}
