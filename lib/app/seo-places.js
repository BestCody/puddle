...
import launchTowns from '@/config/launch-towns.json'
...
// The floor exists so a market with a sloppy bounding box still searches a useful area, but at
// 10km it was larger than several of the towns it applied to, and far larger than any
// neighbourhood: a 1km neighbourhood searching 10km returns downtown rather than the
// neighbourhood, which makes neighbouring pages copies of each other. 3km is small enough that
// separated areas stay distinct, and a place that genuinely has little nearby simply falls under
// the indexable floor instead of borrowing its neighbour's venues.
const MIN_HUB_RADIUS_KM = 3
...
// Towns are carried in their own file because they are a different bet than the metros. A metro
// page competes with OTAs and city magazines; a town page competes with a Yelp search result and
// whatever blog happens to mention it, which is a far softer target and closer to how people
// actually search. They shape identically, so every hub, category page and date page follows.
const MARKETS = [
  ...(Array.isArray(launchMarkets?.markets) ? launchMarkets.markets : []),
  ...(Array.isArray(launchTowns?.markets) ? launchTowns.markets : [])
]
  .map(shapeMarket)
  .filter(Boolean)
