...
import { categoryPlaceholderUrl, staticCatalogueBaseUrl } from './static-catalogue.js'
import { fetchCachedNearbyStaticPlaces } from './static-candidate-cache.js'
import { signStaticCatalogueReference } from './static-catalogue-ref.js'

const MAX_CONTINUATION_CANDIDATES = 500
...
function withStaticMediaIdentity(card, staticItem) {
  if (!staticItem?.static_ref) return card
  const photoUrl = card.photo_url || staticItem.photo_url || null
  const googlePlaceId = card.google_place_id || staticItem.google_place_id || null
  const cardTier = photoUrl ? 3 : googlePlaceId ? 2 : Number(card.card_tier || 1)
  const contentQuality = photoUrl ? Math.max(0.8, Number(card.content_quality_score || 0)) : Number(card.content_quality_score || 0.4)
  return {
    ...card,
    cover_url: photoUrl || card.cover_url || null,
    photo_url: photoUrl,
    has_real_photo: Boolean(photoUrl || card.has_real_photo),
    photo_source: card.photo_source || staticItem.photo_source || null,
    photo_provider: card.photo_provider || staticItem.photo_provider || null,
    photo_attribution: card.photo_attribution || staticItem.photo_attribution || null,
    photo_attribution_url: card.photo_attribution_url || staticItem.photo_attribution_url || null,
    photo_license: card.photo_license || staticItem.photo_license || null,
    google_place_id: googlePlaceId,
    google_place_match_score: card.google_place_match_score ?? staticItem.google_place_match_score ?? null,
    photo_enrichment_status: photoUrl ? 'matched' : googlePlaceId ? 'pending' : card.photo_enrichment_status,
    card_tier: cardTier,
    card_readiness: photoUrl ? 'photo' : googlePlaceId ? 'google' : card.card_readiness,
    ...
    static_media_resolvable: true,
    static_catalogue_source: staticItem.static_catalogue_source || card.static_catalogue_source || null,
    static_catalogue_source_place_id: staticItem.static_catalogue_source_place_id || card.static_catalogue_source_place_id || null,
    static_ref: staticItem.static_ref,
    category_placeholder_url: card.category_placeholder_url || staticItem.category_placeholder_url || null
  }
}
...
async function loadStaticCatalogueNear(session, rawFilters = {}, excludedCount = 0) {
  ...
  if (!Number.isFinite(latitude) || !Number.isFinite(longitude)) {
    return { places: [], fetched: 0, manifest: null, tilesLoaded: 0, tilesRequested: 0, filters, latitude: null, longitude: null, candidateLimit: 0, candidateCache: { status: 'bypass' } }
  }
  const configuredLimit = Number(process.env.STATIC_CATALOGUE_DISCOVERY_LIMIT || 0)
  const requestedLimit = Math.max(filters.limit * 8, 96, excludedCount + filters.limit * 4, Number.isFinite(configuredLimit) ? configuredLimit : 0)
  const limit = Math.max(24, Math.min(MAX_CONTINUATION_CANDIDATES, requestedLimit))
  const nearby = await fetchCachedNearbyStaticPlaces({
  ...
  return {
    ...
    candidateCache: nearby.candidateCache || { status: 'miss' },
...
async function relationalOverlay(session, state) {
  const result = await session.supabase.rpc('r2_discovery_overlay_v1', {
    static_ids: state.places.map((place) => place.contentId).slice(0, MAX_CONTINUATION_CANDIDATES),
...
function buildFeed(session, state, overlay, timings, { excludeIds = [] } = {}) {
  const excluded = new Set((excludeIds || []).map(String))
  const dismissed = new Set((overlay.dismissedIds || []).map(String))
  const staticCards = state.places
    .map((place) => staticCard(place, state.manifest))
    .filter(Boolean)
  const staticById = new Map(staticCards.map((item) => [String(item.content_id), item]))
  const relational = (overlay.locations || [])
    .filter((row) => !excluded.has(String(row.id)))
    .filter((row) => matches(row, state.filters))
    .map((row) => withStaticMediaIdentity(relationalCard(session, row), staticById.get(String(row.id))))
  const relationalIds = new Set(relational.map((item) => item.content_id))
  const staticItems = staticCards
    .filter((item) => !excluded.has(String(item.content_id)))
    .filter((item) => !dismissed.has(String(item.content_id)) && !relationalIds.has(item.content_id))
  ...
  const candidateWindowFull = state.candidateLimit > 0 && state.fetched >= state.candidateLimit
  const hasMore = items.length >= state.filters.limit || (candidateWindowFull && state.candidateLimit < MAX_CONTINUATION_CANDIDATES)
  return {
    requestId,
    ...
    recycled: false,
    emptyReason: state.latitude === null ? 'location_required' : items.length ? null : excluded.size ? 'exhausted' : state.fetched ? 'filters' : 'catalogue_sync_pending',
    continuation: {
      excluded: excluded.size,
      candidateLimit: Number(state.candidateLimit || 0),
      hasMore
    },
    fallback: false, fallbackReason: null,
    rankingVersion: 'r2-overlay-v4-cached',
    experiment: { experiment: 'r2-overlay-v4-cached', variant: 'control', bucket: 0, holdout: false },
    rejections: [],
    ...
    infrastructure: {
      ...
      staticMediaResolvable: items.filter((item) => item.static_catalogue_ephemeral || item.static_media_resolvable).length,
      ...
      candidateCache: state.candidateCache || { status: 'miss' },
      timings
    }
  }
}

export async function getInfrastructureDiscoveryFeedV2(session, rawFilters = {}, { excludeIds = [] } = {}) {
  const normalizedExcludes = [...new Set((excludeIds || []).map(String).filter(Boolean))].slice(0, MAX_CONTINUATION_CANDIDATES)
  const totalStarted = performance.now()
  const catalogueStarted = performance.now()
  const state = await loadStaticCatalogueNear(session, rawFilters, normalizedExcludes.length)
  ...
  if (state.latitude === null || state.longitude === null) {
    return buildFeed(session, state, { dismissedIds: [], interests: session.profile?.interests || [], locations: [] }, {
      catalogueMs: Math.round(catalogueMs * 100) / 100,
      overlayMs: 0,
      totalMs: Math.round((performance.now() - totalStarted) * 100) / 100
    }, { excludeIds: normalizedExcludes })
  }
  const overlayStarted = performance.now()
  const overlay = await relationalOverlay(session, state)
  const overlayMs = performance.now() - overlayStarted
  return buildFeed(session, state, overlay, {
    catalogueMs: Math.round(catalogueMs * 100) / 100,
    overlayMs: Math.round(overlayMs * 100) / 100,
    totalMs: Math.round((performance.now() - totalStarted) * 100) / 100
  }, { excludeIds: normalizedExcludes })
}
...
export async function recordSampledInfrastructureAnalyticsV2(session, feed) {
