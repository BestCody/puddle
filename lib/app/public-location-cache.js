...
async function loadPublicLocation(slug) {
  ...
  try {
    ...
    const moderationPromise = publicSupabase
      ? isLocationSuspended(publicSupabase, row.id)
      : Promise.resolve(false)
    const overlayPromise = publicSupabase
      ? relationalProductOverlay(publicSupabase, location, traceId)
      : Promise.resolve({ location, events: [] })
    const similarPromise = globalSimilar(location, traceId, publicSupabase)
    const [suspended, overlay, similarPlaces] = await Promise.all([
      moderationPromise,
      overlayPromise,
      similarPromise
    ])
    ...
    recordSloObservation('locationDetail', elapsedMs(started), true, { trace_id: traceId, service: 'vercel', serving_mode: 'b2' })
    return { location: overlay.location, similar: [...overlay.events, ...similarPlaces].slice(0, 6) }
  } catch (error) {
...
export const getCachedPublicLocation = unstable_cache(loadPublicLocation, ['public-location-v6'], { revalidate: 300, tags: ['public-locations'] })
