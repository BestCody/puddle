...
async function relatedEvents(supabase, location, traceId) {
  ...
  recordServerLatency('supabase.locationRelatedEvents', elapsedMs(started), SERVER_LATENCY_BUDGET_MS.supabaseQuery, {
    trace_id: traceId,
    service: 'supabase',
    operation: 'locationRelatedEvents'
  })
  return events.map((item) => ({ ...withCardMedia(supabase, item), content_kind: 'event' }))
}
...
async function loadPublicLocationRecommendations(slug) {
  ...
  const [events, similar] = await Promise.all([
    publicSupabase ? relatedEvents(publicSupabase, result.location, traceId) : Promise.resolve([]),
    globalSimilar(result.location, traceId, publicSupabase)
  ])
  return [...events, ...similar].slice(0, 6)
}

const cachedPublicLocationRecommendations = unstable_cache(
  loadPublicLocationRecommendations,
  ['public-location-recommendations-v1'],
