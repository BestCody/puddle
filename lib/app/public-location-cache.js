...
async function loadPublicLocation(slug) {
  ...
  try {
    ...
    if (publicSupabase && await isLocationSuspended(publicSupabase, row.id)) {
      recordSloObservation('locationDetail', elapsedMs(started), true, { trace_id: traceId, service: 'vercel', serving_mode: 'opensearch', moderated: true })
      return null
    }
    let location = globalLocationShape(row)
    let events = []
    if (publicSupabase) {
      const overlay = await relationalProductOverlay(publicSupabase, location, traceId)
      location = overlay.location
      events = overlay.events
    }
    const similarPlaces = await globalSimilar(location, traceId, publicSupabase)
    recordSloObservation('locationDetail', elapsedMs(started), true, { trace_id: traceId, service: 'vercel', serving_mode: 'opensearch' })
