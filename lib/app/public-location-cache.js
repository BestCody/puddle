...
async function relationalProductOverlay(supabase, location, traceId) {
  ...
  const [attached, events] = await Promise.all([
  ...
  return {
    location: { ...location, cover_url: attached?.url || location.cover_url || null },
...
export const getCachedPublicLocation = unstable_cache(loadPublicLocation, ['public-location-v4'], { revalidate: 300, tags: ['public-locations'] })
