...
export async function getGlobalDiscoveryFeed(session, rawFilters = {}, { excludeIds = [] } = {}) {
  ...
  const seenPromise = explicitExcludes.length ? Promise.resolve([]) : seenLocationIds(session)
  const interests = [...new Set(session.profile?.interests || [])]
  const queryStarted = performance.now()
  const searchPromise = searchGlobalLocations({
    latitude,
    longitude,
    distanceKm: filters.distance,
    filters,
    excludeIds: explicitExcludes,
    preferredCategories: interests
  }, { traceId: session.traceId || null })
  const [seen, initialResult] = await Promise.all([seenPromise, searchPromise])
