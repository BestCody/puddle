...
function subdivisionFor(countryCode, rawRegion, explicitRegionCode) {
  const subdivisionMap = countryCode ? SUBDIVISIONS[countryCode] : null
  if (!subdivisionMap) {
    const code = clean(explicitRegionCode, 8).toUpperCase()
    return {
      code: code && /^[A-Z0-9-]{1,8}$/.test(code) ? code : null,
      name: clean(rawRegion, 120) || null
    }
  }

  const candidateCode = clean(explicitRegionCode || rawRegion, 8).toUpperCase()
  if (subdivisionMap[candidateCode]) {
    return { code: candidateCode, name: subdivisionMap[candidateCode] }
  }

  const normalizedName = clean(rawRegion, 120).toLowerCase()
  const nameMatch = Object.entries(subdivisionMap).find(([, name]) => name.toLowerCase() === normalizedName)
  if (nameMatch) return { code: nameMatch[0], name: nameMatch[1] }

  const explicitCode = clean(explicitRegionCode, 8).toUpperCase()
  return {
    code: explicitCode && /^[A-Z0-9-]{1,8}$/.test(explicitCode) ? explicitCode : null,
    name: clean(rawRegion, 120) || null
  }
}
...
export function normalizePlaceGeography(address = {}, record = {}) {
  ...
  const subdivision = subdivisionFor(countryCode, rawRegion, explicitRegionCode)
  const region = rawRegion && countryCode && rawRegion.toUpperCase() === countryCode
    ? null
    : subdivision.name
