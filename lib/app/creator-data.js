...
export async function getEditableEvent(supabase, id) {
  const { data: allowed } = await supabase.rpc('can_manage_event', { target: id })
  if (!allowed) notFound()
  const { data, error } = await supabase.from('events').select('*').eq('id', id).maybeSingle()
  if (error || !data) notFound()
  const [occurrences, revisions, privateDetails] = await Promise.all([
    ...
    queryOr(supabase.from('event_private_details').select('exact_address').eq('event_id', id).maybeSingle(), null)
  ])
  return { ...data, private_address: privateDetails?.exact_address || '', occurrences, revisions }
}
