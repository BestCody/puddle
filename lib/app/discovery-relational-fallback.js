...
function matchesRuntimeFilters(row, filters, now = new Date()) {
  ...
  return Boolean(row.timezone) && isOpenAt(hours, row.timezone, now)
}
...
function card(session, row) {
  ...
  return {
    ...
    candidateSources: ['supabase_relational'],
...
function emptyFeed(session, filters) {
  return {
    ...
    rankingVersion: 'supabase-relational-v1',
    ...
    infrastructure: { catalogue: 'supabase-primary', staticRelease: null, staticFetched: 0, staticServed: 0, relationalServed: 0, staticMediaResolvable: 0, staticMaterialized: 0, staticTilesLoaded: 0, staticTilesRequested: 0, googleUiKitEligible: 0, overlayRpc: 'discovery_primary_overlay_v1', candidateCache: { status: 'not-applicable' }, timings: { catalogueMs: 0, overlayMs: 0, totalMs: 0 } }
  }
}
...
export async function getRelationalDiscoveryFeed(session, rawFilters = {}, { excludeIds = [] } = {}) {
  ...
  const excluded = [...new Set((excludeIds || []).map(String).filter(Boolean))].slice(0, 500)
  const price = /^[1-4]$/.test(filters.price) ? Number(filters.price) : null
  const overlayStarted = performance.now()
  const result = await session.supabase.rpc('discovery_primary_overlay_v1', {
    center_lat: latitude,
    center_lng: longitude,
    radius_m: Math.round(filters.distance * 1000),
    max_rows: PRIMARY_QUERY_LIMIT,
  ...
  return {
    ...
    recycled: false,
    emptyReason: items.length ? null : excluded.length ? 'exhausted' : 'catalogue_sync_pending',
    continuation: {
    ...
    infrastructure: {
      ...
      overlayRpc: 'discovery_primary_overlay_v1', candidateCache: { status: 'not-applicable' },
