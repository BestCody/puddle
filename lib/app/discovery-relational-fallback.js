...
import { categoryPlaceholderUrl } from './static-catalogue.js'
...
function card(session, row) {
  ...
  return {
    ...
    slug: row.slug || null,
    title: row.name,
    summary: row.summary || description(row),
    ...
    source_parent_place_id: row.source_parent_place_id || null,
    duplicate_group_key: row.duplicate_group_key || null,
    catalogue_group_key: row.catalogue_group_key || null,
    ...
    candidateSources: ['relational_fallback'],
    ...
    static_catalogue_source: null,
    static_catalogue_source_place_id: null,
    static_ref: null,
    category_placeholder_url: categoryPlaceholderUrl(row.kind)
  }
}
...
export async function getRelationalDiscoveryFallback(session, rawFilters = {}, { excludeIds = [] } = {}) {
  ...
  if (!Number.isFinite(latitude) || !Number.isFinite(longitude)) {
    return {
      ...
      fallback: true, fallbackReason: 'static_catalogue_unavailable',
      rankingVersion: 'supabase-relational-fallback-v1',
      experiment: { experiment: 'supabase-relational-fallback-v1', variant: 'fallback', bucket: 0, holdout: false },
      rejections: [], personalization: { behavioral: false, friendActivity: false, vector: false, explicitInterestsOnly: true },
      infrastructure: { catalogue: 'supabase-fallback', staticRelease: null, staticFetched: 0, staticServed: 0, relationalServed: 0, staticMediaResolvable: 0, staticMaterialized: 0, staticTilesLoaded: 0, staticTilesRequested: 0, googleUiKitEligible: 0, overlayRpc: 'r2_discovery_overlay_v1', candidateCache: { status: 'b2-unavailable' }, timings: { catalogueMs: 0, overlayMs: 0, totalMs: 0 } }
    }
  }
  ...
  const result = await session.supabase.rpc('r2_discovery_overlay_v1', {
    ...
    radius_m: Math.round(filters.distance * 1000),
    max_rows: Math.min(120, Math.max(24, filters.limit * 6))
  })
  if (result.error) throw result.error
  const overlayMs = performance.now() - overlayStarted
  const overlay = result.data || { dismissedIds: [], interests: [], locations: [] }
  const excluded = new Set((excludeIds || []).map(String))
  const candidates = (overlay.locations || [])
  ...
  return {
    ...
    recycled: false,
    emptyReason: items.length ? null : excluded.size ? 'exhausted' : 'catalogue_sync_pending',
    continuation: { excluded: excluded.size, candidateLimit: candidates.length, hasMore: candidates.length > items.length },
    fallback: true,
    fallbackReason: 'static_catalogue_unavailable',
    rankingVersion: 'supabase-relational-fallback-v1',
    experiment: { experiment: 'supabase-relational-fallback-v1', variant: 'fallback', bucket: 0, holdout: false },
    rejections: [],
    personalization: { behavioral: false, friendActivity: false, vector: false, explicitInterestsOnly: true },
    infrastructure: {
      catalogue: 'supabase-fallback', staticRelease: null, staticFetched: 0, staticServed: 0,
      ...
      overlayRpc: 'r2_discovery_overlay_v1', candidateCache: { status: 'b2-unavailable' },
