...
const BIT_COUNT = ALPHABET.length ** PREFIX_LENGTH
const BITSET_BYTES = Math.ceil(BIT_COUNT / 8)
...
async function loadReady(manifest, manifestKey, { env, fetchFn, signal } = {}) {
  ...
  if (
    ...
    Number(value.bitset_bytes) !== BITSET_BYTES
  ) {
...
function bitsetContainsAll(bitset, indexes) {
  for (const index of indexes) {
    if ((bitset[index >> 3] & (1 << (index & 7))) === 0) return false
  }
  return true
}
...
async function fetchRouteSidecar(manifest, routingObjectKey, { env, fetchFn, signal } = {}) {
  ...
  if (!key) return null
  const maxBytes = Math.max(64 * 1024, Math.min(4 * 1024 * 1024, Number(env.GLOBAL_LOCATION_TEXT_PRUNE_MAX_ROUTE_BYTES) || 2 * 1024 * 1024))
  ...
  if (raw.length > 16 * 1024 * 1024) throw new Error(`Decoded text-prune route ${key} exceeds the runtime budget.`)
...
function metadataRecord(value) {
  ...
  if (!key || typeof value[1] !== 'string') return null
  const bitset = Buffer.from(value[1], 'base64')
  if (bitset.length !== BITSET_BYTES) return null
  return {
    key,
    bitset,
...
export async function pruneTextShardPlan(plan, bounds, query, {
  ...
  if (sidecars.some((value) => value === null)) return null
  ...
  for (const shard of plan.shards) {
    ...
    if (!record || bitsetContainsAll(record.bitset, indexes)) {
      selected.push(shard)
