...
import { directoryTilesForBounds, locationSearchRuntimeConfig, manifestObjectKey } from './location-search-shards.js'
...
function routeObjectKeys(manifest, bounds) {
  ...
  const basePrefix = `${String(manifest?.prefix || '').replace(/\/+$/, '')}/routing`
  return directoryTilesForBounds(bounds, tileDegrees).map(([latIndex, lonIndex]) => {
    const relative = `routing/${latIndex}/${lonIndex}.json.br`
    return prefix === basePrefix ? manifestObjectKey(manifest, relative) : `${prefix}/${latIndex}/${lonIndex}.json.br`
  })
}
...
export async function pruneTextShardPlan(plan, bounds, query, {
  ...
  // Conservative per-pack maxima for every pack visible in the routing tiles.
  // The postings fast path reuses these to prove top-K completeness without
  // touching rows that no query prefix can reach; plain objects keep the shape
  // serializable across diagnostics and reruns.
  const packMaxima = {}
  for (const [key, record] of metadata) {
    packMaxima[key] = {
      maxQuality: record.maxQuality,
      maxPopularity: record.maxPopularity,
      // The hourly photo overlay can add a photo to any canonical location
      // after this immutable derivative was built. Treat every omitted pack
      // as photo-capable for the proof until the overlay is part of the build.
      hasPhoto: record.hasPhoto || photoOverlayActive
    }
  }
  ...
  for (const record of omitted) {
    ...
    const upper = 16 +
      ...
      preferredBonus +
      ((record.hasPhoto || photoOverlayActive) ? Math.max(0, Number(weight.photo) || 0) : 0) +
