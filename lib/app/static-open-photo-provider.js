...
const REQUEST_TIMEOUT_MS = boundedInteger(process.env.OPEN_PHOTO_REQUEST_TIMEOUT_MS, 3_000, { min: 1_000, max: 10_000 })
...
async function fetchWithRetry(url, options = {}, policy = {}) {
  ...
  const timeoutMs = boundedInteger(policy.timeoutMs, REQUEST_TIMEOUT_MS, { min: 500, max: 10_000 })
  let lastError = null
...
async function wikimediaCandidates(location) {
  ...
  const payload = await fetchJson(url, {}, {
    provider: 'wikimedia-commons', maxAttempts: 1,
    minIntervalMs: WIKIMEDIA_MIN_INTERVAL_MS, baseDelayMs: 500, timeoutMs: 2_500
  })
...
async function mapillaryCandidates(location) {
  ...
  const payload = await fetchJson(url, {}, {
    provider: 'mapillary', maxAttempts: 1, baseDelayMs: 500, timeoutMs: 2_000
  })
...
async function kartaviewCandidates(location) {
  ...
  const payload = await fetchJson(url, {}, {
    provider: 'kartaview', maxAttempts: 1, baseDelayMs: 500, timeoutMs: 1_500
  })
...
export async function findStaticOpenPhotoCandidates(location, {
  ...
  const providers = providerOrderForCategory(location.kind)
  const outcomes = await Promise.all(providers.map(async (provider) => {
    try {
      return { provider, candidates: await candidatesFor(provider, location), error: null }
    } catch (error) {
  ...
  for (const outcome of outcomes) {
    if (outcome.error) {
      failures.push(`${outcome.provider} lookup: ${outcome.error.message}`)
      continue
    }
    const eligible = outcome.candidates
      .filter((candidate) => candidate.score >= minScore)
      .slice(0, maxCandidatesPerProvider)
