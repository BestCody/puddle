...
import { b2Configuration, b2PublicUrl, putB2Object } from './b2-s3.js'

export function openPhotoB2Limits(env = process.env) {
...
export async function transformOpenPhotoForB2(source, limits = openPhotoB2Limits()) {
  const attempts = [
...
export async function storeOpenPhotoInB2(admin, source, {
  config = b2Configuration(),
  limits = openPhotoB2Limits()
} = {}) {
  ...
  const transformed = await transformOpenPhotoForB2(source, limits)
  ...
  const remoteUrl = duplicate?.public_url || b2PublicUrl(key, config)
  if (!remoteUrl) throw new Error('Backblaze B2 download URL is unavailable.')

  if (!duplicate) {
    await putB2Object(key, transformed.body, {
  ...
  if (!media?.id) throw new Error('Could not register the Backblaze B2 media object.')
  return {
