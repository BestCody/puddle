...
function isStaticResolvable(item) {
  return Boolean(
    (item?.static_catalogue_ephemeral || item?.static_media_resolvable) &&
    item?.static_ref &&
    item?.content_id
  )
}
...
function hasKnownMedia(item) {
  return Boolean(item?.photo_url || item?.cover_url)
}

function photoStatus(state, hasPhoto) {
  if (hasPhoto || state === 'open_photo_found') return 'matched'
  if (state === 'google_server_photo' || state === 'google_client_lookup' || state === 'google_matched') return 'processing'
...
function mergeMedia(item, result) {
  ...
  return {
    ...
    google_server_photo: Boolean(result.google_server_photo || item.google_server_photo),
    google_photo_proxy_url: result.google_photo_proxy_url || item.google_photo_proxy_url || null,
    google_client_lookup: Boolean(result.google_client_lookup || item.google_client_lookup),
    google_lookup_min_score: result.google_lookup_min_score ?? item.google_lookup_min_score ?? null,
...
function cacheableMedia(result) {
  return Boolean(
    result?.photo_url ||
    result?.google_photo_proxy_url ||
    result?.google_place_id ||
    result?.google_client_lookup ||
    ['open_photo_found', 'google_matched', 'google_server_photo', 'google_client_lookup', 'no_match'].includes(String(result?.state || ''))
  )
}

function shouldCacheMedia(mode, result) {
  if (!cacheableMedia(result)) return false
  // Open-photo lookahead may discover an already-verified Google Place ID, but it must
  // not satisfy the later visible-card request. The full request is what mints the
  // transient authenticated Google photo proxy capability.
  if (mode === 'open_only' && result?.state === 'google_matched' && !result?.photo_url) return false
  return true
}
...
async function requestStaticMedia(item, mode) {
  ...
  const task = (async () => {
    ...
    if (response.ok && shouldCacheMedia(mode, result)) resolvedMediaCache.set(key, result)
...
export function useStaticMediaResolution(sourceItem) {
  ...
  useEffect(() => {
    ...
    sourceItem?.static_media_resolvable,
