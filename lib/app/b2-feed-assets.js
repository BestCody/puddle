...
async function authorizeField(value, config, fetchImpl, now) {
  const key = b2ObjectKeyFromUrl(value, config)
  if (!key) return { value, key: null }
  return {
    value: await authorizeB2DownloadUrl(value, { config, fetchImpl, now }),
    key
  }
}

async function authorizeItem(item, config, fetchImpl, now) {
  const [cover, photo, placeholder, gallery] = await Promise.all([
    authorizeField(item?.cover_url, config, fetchImpl, now),
    authorizeField(item?.photo_url, config, fetchImpl, now),
    authorizeField(item?.category_placeholder_url, config, fetchImpl, now),
    Promise.all((Array.isArray(item?.photo_urls) ? item.photo_urls : []).map((value) => authorizeField(value, config, fetchImpl, now)))
  ])
  const keys = {
    cover: cover.key,
    photo: photo.key,
...
export async function authorizeDiscoveryFeedB2Assets(feed, {
  ...
  const items = await Promise.all(feed.items.map((item) => authorizeItem(item, config, fetchImpl, now)))
