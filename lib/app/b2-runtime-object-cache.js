...
const DEFAULT_MANIFEST_TTL_SECONDS = 300
...
const CACHE_NAMESPACE = 'b2-search-v3'
...
function activeKey(env) {
  const dataPrefix = normalizedKey(env.B2_DATA_PREFIX || 'data')
  return normalizedKey(env.GLOBAL_LOCATION_SEARCH_MANIFEST_KEY || `${dataPrefix}/search/active.json`)
}
...
export function b2RuntimeObjectCachePolicy(key, env = process.env) {
  ...
  const manifestPattern = new RegExp(`^${escaped(searchPrefix(env))}\/schema=v1\/snapshot=[^/]+\/manifest(?:-[^/]+)?\\.json$`)
  if (manifestPattern.test(objectKey)) {
    return {
      kind: 'manifest',
      ttl: integer(env.GLOBAL_LOCATION_SEARCH_MANIFEST_RUNTIME_CACHE_TTL_SECONDS, DEFAULT_MANIFEST_TTL_SECONDS, 5, 300)
    }
  }
...
export function isImmutableB2SearchObject(key, env = process.env) {
  const objectKey = normalizedKey(key)
  const prefix = escaped(searchPrefix(env))
  return new RegExp(`^${prefix}\/schema=v1\/snapshot=[^/]+\/`).test(objectKey) ||
    new RegExp(`^${prefix}\/objects\/v1\/sha256=[0-9a-f]{2}\/[0-9a-f]{64}$`).test(objectKey) ||
    new RegExp(`^${prefix}\/photo-overlay-v2\/objects\/sha256=[0-9a-f]{64}\/bucket=[0-9a-f]{2}\.json\.br$`).test(objectKey)
}
