...
export async function getDateMatchSnapshot(session, rawToken) {
  ...
  const now = new Date()
  const [deckResult, membersResult, itemsResult, swipesResult, matchesResult, feedbackResult, revealsResult] = await Promise.all([
    session.supabase.from('date_match_decks').select('id,created_by,title,status,center_latitude,center_longitude,created_at,updated_at,expires_at').eq('id', deckId).maybeSingle(),
    session.supabase.from('date_match_members').select('profile_id,role,joined_at').eq('deck_id', deckId).order('joined_at'),
    session.supabase.from('date_match_items').select('location_id,sort_order,is_puddle_pick,locations(id,slug,name,summary,kind,timezone,price_level,accessibility,amenities,opening_hours,latitude,longitude,cover_path,status,visibility,has_private_address)').eq('deck_id', deckId).order('sort_order'),
    session.supabase.from('date_match_swipes').select('location_id,choice,note,updated_at').eq('deck_id', deckId).eq('profile_id', session.user.id),
    session.supabase.from('date_match_matches').select('location_id,strength,status,matched_at,planned_for,updated_at').eq('deck_id', deckId).order('strength', { ascending: false }),
    session.supabase.from('date_match_feedback').select('location_id,happened,rating,updated_at').eq('deck_id', deckId).eq('profile_id', session.user.id),
    session.supabase.rpc('date_match_reveals_v1', { target_deck: deckId })
  ])

  const deck = deckResult.data
  if (!deck || itemsResult.error) return null
  const rows = (itemsResult.data || []).filter((row) => row.locations?.status === 'published' && row.locations?.visibility === 'public' && !row.locations?.has_private_address)
  const photos = await loadLocationPhotos(session.supabase, rows.map((row) => row.location_id), now)
  const reveals = new Map((revealsResult.data || []).map((row) => [row.location_id, row]))
  const items = rows.map((row) => formatLocation(session.supabase, row, deck, photos.get(row.location_id), reveals.get(row.location_id), now))
  ...
  const matches = (matchesResult.data || []).map((match) => ({ ...match, feedback: feedback.get(match.location_id) || null }))
