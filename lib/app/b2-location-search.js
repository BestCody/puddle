...
export async function searchB2GlobalLocations({
  ...
  if (query.normalized) {
    ...
    try {
      projection = await fetchTextProjectionCore(plan, { manifest, manifestKey, env, fetchFn, signal })
    } catch (error) {
  ...
  return {
    ...
    diagnostics: {
      ...
      textProjection: Boolean(projection),
      textProjectionFallback: projectionFallback,
      timings: { manifestMs, routingMs, fetchDecodeMs, scoreMs, hydrateMs }
    }
  }
}
