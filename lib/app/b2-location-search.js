...
export async function searchB2GlobalLocations({
  ...
  async function photoOverlayForIds(ids) {
    ...
    return getPhotoSearchOverlay({ manifest, manifestKey, ids, env, fetchFn, signal })
  }
  if (query.normalized) {
  ...
  return {
    ...
    diagnostics: {
      ...
      photoFirst,
      photoOverlay: photoOverlayPointer?.active ? { active: true, photoCount: photoOverlayPointer.photoCount } : { active: false, photoCount: 0 },
