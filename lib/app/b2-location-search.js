...
function matchesStructuredFilters(row, filters = {}) {
  ...
  if (/^[1-4]$/.test(String(filters.price || '')) && Number(row.price_level) !== Number(filters.price)) return false
  const amenity = text(filters.amenity, 100).toLowerCase()
  if (amenity) {
    const amenities = Array.isArray(row.amenities) ? row.amenities : []
    if (!amenities.some((value) => String(value || '').toLowerCase() === amenity)) return false
  }
...
export async function searchB2GlobalLocations({
  ...
  for (const row of documents) {
    if (!row?.id || !published(row) || excluded.has(String(row.id)) || !matchesStructuredFilters(row, filters)) continue
