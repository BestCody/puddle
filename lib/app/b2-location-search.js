...
import { b2RuntimeObjectCacheEnabled } from './b2-runtime-object-cache.js'
import { getPhotoSearchOverlay, photoObjectFromOverlay } from './b2-photo-search-overlay.js'
import {
...
function createPhotoAwareTopK(limit, photoFirst = false) {
  const ranked = createTopK(limit)
  const photos = photoFirst ? createTopK(limit) : null

  return {
    push(entry) {
      ranked.push(entry)
      if (photos && entry.hasPhoto) photos.push(entry)
    },
    values() {
      if (!photos) return ranked.values()
      const photoValues = photos.values()
      if (photoValues.length >= limit) return photoValues

      const selected = [...photoValues]
...
function indexedPhoto(value) {
  if (typeof value === 'string' && value) return { content_hash: value }
  if (value && typeof value === 'object' && !Array.isArray(value) && value.content_hash) return value
  return null
}

function effectivePhoto(id, indexed, overlay) {
  if (!overlay?.active) return indexedPhoto(indexed)
  return photoObjectFromOverlay(overlay.photos.get(String(id)))
}

function mergePhoto(row, id, overlay) {
  if (!overlay?.active) return row
  return { ...row, primary_photo: photoObjectFromOverlay(overlay.photos.get(String(id))) }
}
...
export async function searchB2GlobalLocations({
  ...
  const photoFirst = Boolean(preferPhoto)
  ...
  // The overlay pointer is tiny and cached independently of the photo payload.
  // Resolve it before text pruning so a newly materialized photo can never be
  // hidden by a stale derivative's pack maxima.
  const photoOverlayPointerPromise = getPhotoSearchOverlay({ manifest, manifestKey, ids: [], env, fetchFn, signal })
  ...
  const photoOverlayPointer = await photoOverlayPointerPromise
  let photoOverlayPossibleCount = 0
  let photoOverlayMatchedCount = 0
  async function photoOverlayForIds(ids) {
    if (!photoOverlayPointer?.active) return null
    const result = await getPhotoSearchOverlay({ manifest, manifestKey, ids, env, fetchFn, signal })
    photoOverlayPossibleCount = Math.max(photoOverlayPossibleCount, result?.possibleCount || 0)
    photoOverlayMatchedCount = Math.max(photoOverlayMatchedCount, result?.matchedCount || 0)
    return result
  }
  if (query.normalized) {
    ...
    prune = await pruneTextShardPlan(plan, bounds, query, {
      ...
      photoOverlayActive: Boolean(photoOverlayPointer?.active),
      env,
      fetchFn,
      signal
    })
  ...
  if (prune && prefixCodes) {
    ...
    postings = await fetchTextPostingsForPlan(servingPlan, bounds, { manifest, manifestKey, env, fetchFn, signal })
  ...
  async function scorePostingsRefs(refs, ready) {
    ...
    const top = createPhotoAwareTopK(limit, photoFirst)
    ...
    for (const group of groups.values()) {
      for (const ref of group.refs) {
        ...
        eligible.push({
          ...
          textScore,
          distanceM,
          indexedPhoto: row[d.PRIMARY_PHOTO]?.[0] || null
        })
      }
    }
    const overlay = await photoOverlayForIds(eligible.map((entry) => entry.id))
    for (const entry of eligible) {
      const photo = effectivePhoto(entry.id, entry.indexedPhoto, overlay)
      const hasPhoto = Boolean(photo?.content_hash)
      const scoringRow = {
        quality_score: Number(entry.row[DETAIL_INDEX.QUALITY_SCORE] || 0),
        popularity_score: Number(entry.row[DETAIL_INDEX.POPULARITY_SCORE] || 0),
        category: entry.row[DETAIL_INDEX.CATEGORY],
        // Photo bonus must mirror the scan paths exactly so rankings are
        // byte-identical no matter which serving mode produced them.
        primary_photo: photo
      }
      const score = scoreLocation(scoringRow, {
      ...
      top.push({ id: entry.id, sourceKey: entry.sourceKey, rowIndex: entry.rowIndex, score, distanceM: entry.distanceM, hasPhoto })
      documents.set(`${entry.sourceKey}:${entry.rowIndex}`, mergePhoto(materializeDetailRow(entry.row), entry.id, overlay))
    }
  ...
  async function executePlan(targetPlan) {
    ...
    const fetchStarted = performance.now()
    // The active projection core contains every field needed for geographic and
    // structured ranking, not only normalized text. Reuse it for all radius
    // searches so cold discovery avoids downloading and decoding full documents;
    // only the small winning detail chunks are hydrated afterward. Null means the
    // derivative is not activated. Once activated, corruption still fails loudly.
    ...
    if (projection) {
      ...
      for (const pack of projection.packs) {
        for (let rowIndex = 0; rowIndex < pack.rows.length; rowIndex += 1) {
          ...
          // NOTE: no substring pre-reject here. Fuzzy branches (banded edit
          // distance / trigram overlap) can match names that do not contain any
          // query token as a substring, so only the full scorer may reject.
          const textScore = query.normalized
            ? scoreNormalizedTextFields(
          ...
          eligible.push({
            ...
            textScore,
            distanceM,
            indexedPhoto: compact[p.PHOTO_HASH] || null
          })
        }
      }
      const overlay = await photoOverlayForIds(eligible.map((entry) => entry.id))
      for (const entry of eligible) {
        ...
        const hasPhoto = Boolean(photo?.content_hash)
        const scoringRow = {
          quality_score: entry.compact[p.QUALITY_SCORE],
          popularity_score: entry.compact[p.POPULARITY_SCORE],
          category: entry.compact[p.CATEGORY],
          primary_photo: photo
        }
        const score = scoreLocation(scoringRow, {
          textScore: entry.textScore,
      ...
      candidates = winners.map((winner, index) => {
        ...
        return { ...mergePhoto(row, winner.id, overlay), distance_m: winner.distanceM, search_score: winner.score }
      })
    }
    ...
    if (!candidates) {
      ...
      for (const row of documents) {
        ...
        if (!Number.isFinite(distanceM) || distanceM > maxDistanceM) continue
        eligible.push({ row, textScore, distanceM, indexedPhoto: row.primary_photo || null })
      }
      const overlay = await photoOverlayForIds(eligible.map((entry) => entry.row.id))
      for (const entry of eligible) {
        const photo = effectivePhoto(entry.row.id, entry.indexedPhoto, overlay)
        const row = overlay?.active ? { ...entry.row, primary_photo: photo } : entry.row
        const score = scoreLocation(row, {
          textScore: entry.textScore,
          distanceM: entry.distanceM,
          maxDistanceM,
          preferredCategories: preferred,
        ...
        top.push({ id: row.id, row, score, distanceM: entry.distanceM, hasPhoto: Boolean(photo?.content_hash) })
      }
  ...
  if (postings) {
    ...
    postingProof = { bound: rounded(bound), cutoff: rounded(cutoff), complete }
    if (!complete) {
      const postingsExecution = execution
      execution = await executePlan(servingPlan)
      textMode = 'core-scan-rerun'
      textPostingsRerun = true
      // Keep the fast-path attempt visible in diagnostics instead of letting
      // the rerun overwrite it.
      execution.postingRefs = postingsExecution.postingRefs
    }
  }
  ...
  return {
    ...
    backend: 'b2',
    diagnostics: {
      runtimeCache: b2RuntimeObjectCacheEnabled({ env, fetchFn }),
      ...
      textPostingsLayout: postings?.ready?.layout ?? null,
      textPostingProof: postingProof,
      photoFirst,
      photoOverlay: photoOverlayPointer?.active
        ? { active: true, photoCount: photoOverlayPointer.photoCount, possibleCount: photoOverlayPossibleCount, matchedCount: photoOverlayMatchedCount }
        : { active: false, photoCount: 0, possibleCount: 0, matchedCount: 0 },
...
export async function searchB2GlobalLocationsInViewport(input = {}, { env = process.env, fetchFn = fetch } = {}) {
  ...
  const { active, manifest, manifestKey } = await getActiveSearchManifest({ env, fetchFn, signal })
  const manifestMs = roundedMs(manifestStarted)
  const photoOverlayPointer = await getPhotoSearchOverlay({ manifest, manifestKey, ids: [], env, fetchFn, signal })
  ...
  const scoreStarted = performance.now()
  const weights = rankingWeights(env)
  const top = createPhotoAwareTopK(limit, true)
  const eligible = []
  for (const row of documents) {
    if (!row?.id || !published(row) || !pointInBounds(row.latitude, row.longitude, bounds)) continue
    eligible.push({ row, indexedPhoto: row.primary_photo || null })
  }
  const overlay = photoOverlayPointer?.active
    ? await getPhotoSearchOverlay({ manifest, manifestKey, ids: eligible.map((entry) => entry.row.id), env, fetchFn, signal })
    : null
  for (const entry of eligible) {
    const photo = effectivePhoto(entry.row.id, entry.indexedPhoto, overlay)
    const row = overlay?.active ? { ...entry.row, primary_photo: photo } : entry.row
    const score = scoreLocation(row, { weights })
    top.push({ id: row.id, row, score, distanceM: Number.POSITIVE_INFINITY, hasPhoto: Boolean(photo?.content_hash) })
  }
  ...
  return {
    ...
    diagnostics: plan ? {
      ...
      photoFirst: true,
       photoOverlay: photoOverlayPointer?.active
         ? { active: true, photoCount: photoOverlayPointer.photoCount, possibleCount: overlay?.possibleCount || 0, matchedCount: overlay?.matchedCount || 0 }
         : { active: false, photoCount: 0, possibleCount: 0, matchedCount: 0 },
      timings
    } : {
      coarse: true,
...
export async function getB2GlobalLocationsByIds(ids = [], { env = process.env, fetchFn = fetch } = {}) {
  const config = locationSearchRuntimeConfig(env)
  const signal = AbortSignal.timeout(config.timeoutMs)
  const { manifest, manifestKey } = await getActiveSearchManifest({ env, fetchFn, signal })
  const rows = await getLocationsByIdsFromShards(ids, { env, fetchFn, signal, manifest })
  const overlay = await getPhotoSearchOverlay({ manifest, manifestKey, ids: rows.map((row) => row?.id), env, fetchFn, signal })
  return rows.map((row) => mergePhoto(row, row?.id, overlay))
}
...
export async function getB2GlobalLocationBySlug(slug, { env = process.env, fetchFn = fetch } = {}) {
  ...
  const signal = AbortSignal.timeout(config.timeoutMs)
  const { manifest, manifestKey } = await getActiveSearchManifest({ env, fetchFn, signal })
  const row = await getLocationBySlugFromShards(slug, { env, fetchFn, signal, manifest })
  if (!row) return null
  const overlay = await getPhotoSearchOverlay({ manifest, manifestKey, ids: [row.id], env, fetchFn, signal })
  return mergePhoto(row, row.id, overlay)
}
