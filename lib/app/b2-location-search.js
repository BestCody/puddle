...
export async function searchB2GlobalLocations({
  ...
  const firstToken = Array.isArray(query?.tokens) && query.tokens.length ? String(query.tokens[0]) : ''
  let postings = null
  ...
  // True when `token` appears anywhere inside any raw text field of a compact
  // core row. Query tokens are already lowercase from prepareTextQuery.
  const CORE_TOKEN_FIELDS = [TEXT_CORE_INDEX.NAME, TEXT_CORE_INDEX.CATEGORY, TEXT_CORE_INDEX.CITY, TEXT_CORE_INDEX.NEIGHBORHOOD, TEXT_CORE_INDEX.ADDRESS]

  function compactContainsToken(compact, token) {
    for (let index = 0; index < CORE_TOKEN_FIELDS.length; index += 1) {
      const value = compact[CORE_TOKEN_FIELDS[index]]
      if (typeof value === 'string' && value.toLowerCase().includes(token)) return true
    }
    const aliases = compact[TEXT_CORE_INDEX.ALIASES]
    if (Array.isArray(aliases)) {
      for (const alias of aliases) {
        if (typeof alias === 'string' && alias.toLowerCase().includes(token)) return true
      }
    }
    return false
  }
    ...
    if (projection) {
      ...
      for (const pack of projection.packs) {
        for (let rowIndex = 0; rowIndex < pack.rows.length; rowIndex += 1) {
          ...
          // Sound pre-reject: every scoring branch needs the first query token
          // to appear somewhere in the raw text fields (as a token prefix or as
          // part of the full normalized substring). If it appears in none of
          // them, textScore is provably zero and the expensive scorer is skipped.
          if (firstToken && !compactContainsToken(compact, firstToken)) continue
          const textScore = scoreNormalizedTextFields(
