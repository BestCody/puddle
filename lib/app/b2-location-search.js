  ...
  async function scorePostingsRefs(refs, ready) {
    ...
    for (const group of groups.values()) {
      for (const ref of group.refs) {
        ...
        const hasPhoto = Boolean(row[d.PRIMARY_PHOTO]?.[0])
        const scoringRow = {
          ...
          primary_photo: hasPhoto ? { content_hash: row[d.PRIMARY_PHOTO][0] } : null
        }
        const score = scoreLocation(scoringRow, { textScore, distanceM, maxDistanceM, preferredCategories: preferred, weights })
        top.push({ id, sourceKey: ref.sourceKey, rowIndex: ref.rowIndex, score, distanceM, hasPhoto })
  ...
  async function executePlan(targetPlan) {
    ...
    if (projection) {
      ...
      for (const pack of projection.packs) {
        for (let rowIndex = 0; rowIndex < pack.rows.length; rowIndex += 1) {
          ...
          const distanceM = haversineDistanceMeters(lat, lon, rowLat, rowLon)
          if (!Number.isFinite(distanceM) || distanceM > maxDistanceM) continue
          const hasPhoto = Boolean(compact[p.PHOTO_HASH])
          const scoringRow = {
            quality_score: compact[p.QUALITY_SCORE],
            popularity_score: compact[p.POPULARITY_SCORE],
            category: compact[p.CATEGORY],
            primary_photo: hasPhoto ? { content_hash: compact[p.PHOTO_HASH] } : null
          }
          const score = scoreLocation(scoringRow, { textScore, distanceM, maxDistanceM, preferredCategories: preferred, weights })
          top.push({ id, sourceKey: pack.sourceKey, rowIndex, score, distanceM, hasPhoto })
        }
      }
    ...
    if (!candidates) {
      ...
      for (const row of documents) {
        ...
        top.push({ id: row.id, row, score, distanceM, hasPhoto: Boolean(row.primary_photo?.content_hash) })
      }
