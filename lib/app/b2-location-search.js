...
import {
  fetchTextProjectionRows,
  materializeTextProjectionRow,
  TEXT_PROJECTION_INDEX
...
function matchesProjectionStructuredFilters(row, filters) {
...
function normalizedProjectionValue(row, normalizedIndex, rawIndex) {
...
function normalizedProjectionAliases(row) {
  const p = TEXT_PROJECTION_INDEX
...
export async function searchB2GlobalLocations({
  ...
  let projectionRows = null
  let projectionFallback = false
  if (query.normalized) {
    try {
      projectionRows = await fetchTextProjectionRows(plan, { manifest, manifestKey, env, fetchFn, signal })
    } catch (error) {
  ...
  if (projectionRows) {
    const p = TEXT_PROJECTION_INDEX
    for (const compact of projectionRows) {
      ...
      if (!pointInBounds(rowLat, rowLon, bounds) || !matchesProjectionStructuredFilters(compact, structured)) continue
      const textScore = scoreNormalizedTextFields(
        normalizedProjectionValue(compact, p.NORMALIZED_NAME, p.NAME),
        normalizedProjectionAliases(compact),
        normalizedProjectionValue(compact, p.NORMALIZED_CATEGORY, p.CATEGORY),
        normalizedProjectionValue(compact, p.NORMALIZED_CITY, p.CITY),
        normalizedProjectionValue(compact, p.NORMALIZED_NEIGHBORHOOD, p.NEIGHBORHOOD),
        normalizedProjectionValue(compact, p.NORMALIZED_ADDRESS, p.ADDRESS),
        query
      )
  ...
  const decodedCandidates = projectionRows?.length ?? documents?.length ?? 0
  return {
    ...
    diagnostics: {
      ...
      textProjection: Boolean(projectionRows),
      textProjectionFallback: projectionFallback,
      timings: { manifestMs, routingMs, fetchDecodeMs, scoreMs }
    }
  }
}
