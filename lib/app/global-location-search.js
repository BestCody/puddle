...
export async function getGlobalLocationBySlug(slug, { fetchFn = fetch, env = process.env, traceId = null } = {}) {
  const config = globalLocationSearchConfig(env)
...
export async function getGlobalLocationsByIds(ids = [], { fetchFn = fetch, env = process.env, traceId = null } = {}) {
  const config = globalLocationSearchConfig(env)
