...
function endpointFromEnv(env = process.env) {
  ...
  if (url.protocol !== 'https:' && !(local && url.protocol === 'http:')) {
    throw new Error('Global location search URL must use HTTPS outside local development.')
  }
...
export function isGlobalLocationSearchConfigured(env = process.env) {
  const config = globalLocationSearchConfig(env)
...
function viewportGeoFilter(bounds) {
  ...
  return {
    bool: {
      should: [box(bounds.west, 180), box(-180, bounds.east)],
      minimum_should_match: 1
    }
  }
}

export function buildGlobalLocationViewportSearchBody({ north, south, east, west, zoom, candidateLimit } = {}) {
  const bounds = normalizeGlobalLocationViewport({ north, south, east, west, zoom })
  const limit = integer(candidateLimit, viewportLocationLimit(bounds.zoom), 1, 250)
  const bool = {
    filter: [viewportGeoFilter(bounds), { term: { status: 'published' } }],
    should: [{ exists: { field: 'primary_photo.url' } }],
  ...
  return {
    ...
    _source: [
      'id', 'slug', 'name', 'summary', 'description', 'category', 'country', 'country_code',
      'region', 'city', 'neighborhood', 'latitude', 'longitude', 'quality_score', 'popularity_score',
...
export function buildGlobalLocationSearchBody({
  latitude,
  longitude,
  distanceKm,
  filters = {},
  ...
  bool.filter.push({ geo_distance: { distance: `${Math.min(20_040, distance)}km`, location: { lat, lon } } })
  ...
  bool.should.push({ exists: { field: 'primary_photo.url' } })
...
function normalizeHit(hit) {
  ...
  const distance = Array.isArray(hit?.sort)
    ? hit.sort.find((value, index) => index > 0 && Number.isFinite(Number(value)))
    : null
  return {
    ...source,
...
async function executeSearch(body, { fetchFn = fetch, env = process.env, includeDistance = true } = {}) {
  const config = globalLocationSearchConfig(env)
...
export async function searchGlobalLocations(input, options = {}) {
  const config = globalLocationSearchConfig(options.env || process.env)
  const body = buildGlobalLocationSearchBody({ ...input, candidateLimit: input?.candidateLimit || config.candidateLimit })
  return executeSearch(body, options)
}

export async function searchGlobalLocationsInViewport(input, options = {}) {
  const body = buildGlobalLocationViewportSearchBody(input)
  return executeSearch(body, { ...options, includeDistance: false })
}

export async function getGlobalLocationBySlug(slug, { fetchFn = fetch, env = process.env } = {}) {
  const config = globalLocationSearchConfig(env)
  ...
  const payload = await response.json()
  return normalizeHit(payload?.hits?.hits?.[0] || null)?.id ? normalizeHit(payload.hits.hits[0]) : null
}

export async function getGlobalLocationsByIds(ids = [], { fetchFn = fetch, env = process.env } = {}) {
  const config = globalLocationSearchConfig(env)
