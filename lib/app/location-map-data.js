...
export async function getLocationMapSnapshot(session) {
  ...
  const personalLocations = personalLocationIds.length ? await queryOr(
    session.supabase
      ...
      .limit(personalLocationIds.length)
  ) : []
  ...
  const photoRows = locationIds.length ? await queryOr(
    session.supabase
      ...
      .eq('status', 'approved')
      // Database migration 10070 enforces one approved photo per location; the explicit
      // limit keeps this enrichment bounded even if that invariant is ever bypassed.
      .limit(locationIds.length)
  ) : []

  const photosByLocation = new Map()
  for (const row of photoRows) {
    if (!photosByLocation.has(row.location_id)) photosByLocation.set(row.location_id, [row])
  }
