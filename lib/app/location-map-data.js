...
export async function getLocationMapSnapshot(session) {
  ...
  const [personalLocations, heatmapRows] = await Promise.all([
    personalLocationIds.length ? queryOr(
      session.supabase
        .from('locations')
  ...
  const profileCenter = Number.isFinite(profileLatitude) && Number.isFinite(profileLongitude)
    ...
    : null
  const centerSource = points.length ? points : heatmap
  const center = profileCenter || (centerSource.length ? {
