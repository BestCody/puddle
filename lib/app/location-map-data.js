...
async function searchCatalogue(session, searchTerm) {
  if (!searchTerm) return { rows: [], backend: null }

  if (isGlobalLocationSearchConfigured()) {
    try {
      ...
      const result = await searchGlobalLocations({
        ...
        preferredCategories: session.profile?.interests || [],
        candidateLimit: 50
      })
      return {
        rows: result.candidates.map((row) => ({
          ...row,
      ...
      console.warn('Map catalogue OpenSearch search failed; using relational fallback.', { status: error?.status || null })
    }
  }

  const rows = await rpcOr(session, 'public_map_location_search_v1', { search_term: searchTerm, result_limit: 50 }, [])
  return { rows, backend: 'supabase_fallback' }
}
...
export async function getLocationMapSnapshot(session, searchQuery = '') {
  ...
  const catalogueSearch = normalizedQuery ? searchCatalogue(session, normalizedQuery) : Promise.resolve({ rows: [], backend: null })
  const [personalLocations, catalogueResult, heatmapRows] = await Promise.all([
  ...
  const catalogueLocations = catalogueResult.rows
  ...
  const points = locations
    ...
    .map((location) => {
      ...
      const kind = location.kind || location.category || 'location'
      return {
        id: location.id,
        location_id: location.id,
        title: location.name,
        summary: location.summary || location.description || `A ${String(kind).replaceAll('_', ' ')} in ${location.neighborhood || location.city || 'your area'}.`,
        category: kind,
        ...
        photo_url: location.primary_photo?.url || publicMediaUrl(session.supabase, location.cover_path) || providerPhotoPath(chosenPhoto?.id) || null,
  ...
  return {
    ...
    searchBackend: normalizedQuery ? catalogueResult.backend : null
  }
}
