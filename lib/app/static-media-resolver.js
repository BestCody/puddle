...
const OPEN_PHOTO_MISS_PREFIX = 'open_photo_miss:v2'
...
export function staticMediaResolverConfiguration(env = process.env) {
  ...
  return {
    ...
    googleTimeoutMs: boundedInteger(env.GOOGLE_PLACE_MATCH_TIMEOUT_MS, 5_000, { minimum: 1_500, maximum: 15_000 }),
...
async function resolutionRecord(admin, reference) {
  const result = await admin
...
function openPhotoAlreadyChecked(record) {
  return record?.state === 'pending' && String(record?.last_error || '').startsWith(OPEN_PHOTO_MISS_PREFIX)
}
...
async function finishOpenPhotoPrefetch(admin, reference, token, failures = []) {
  const message = [OPEN_PHOTO_MISS_PREFIX, ...failures.slice(0, 3)].join(': ').slice(0, 500)
  const result = await admin
    .from('static_media_resolution_states')
    .update({
      state: 'pending',
...
async function tryOpenPhoto(admin, place, config, b2Writer) {
  const found = await findStaticOpenPhotoCandidates(place, { maxCandidatesPerProvider: 3 })
...
function safeGoogleFailure(error) {
  ...
  if (/timeout|aborted/i.test(message) || ['AbortError', 'TimeoutError'].includes(error?.name)) return 'Google Places lookup timed out.'
  if (/^Google Places returned \d{3}\.$/.test(message)) return message
  return 'Google Places lookup failed.'
}
...
async function resolveGoogle(admin, place, config) {
  ...
  let payload
  try {
    payload = await searchGoogle(place, config)
  } catch (error) {
    return { state: 'temporary_failure', error: safeGoogleFailure(error) }
  }
...
export async function resolveStaticCatalogueMedia(reference, {
  ...
  if (relation(existing?.media_objects) || (!openOnly && existing?.google_place_id)) {
    return { status: 200, payload: staticAssetMediaPayload(existing) }
  }

  const previous = await resolutionRecord(admin, reference)
  const openPhotoChecked = openPhotoAlreadyChecked(previous)
  if (openOnly && openPhotoChecked) {
    return {
      status: 200,
      payload: { ...staticAssetMediaPayload(null, 'open_photo_miss'), retryable: false }
    }
  }
  ...
  if (!claim.claimed) {
    ...
    if (relation(refreshed?.media_objects) || (!openOnly && refreshed?.google_place_id)) {
  ...
  try {
    ...
    const photo = openPhotoChecked && !openOnly
      ? { matched: false, failures: [] }
      : await tryOpenPhoto(admin, place, config, b2Writer)
    if (photo.matched) {
    ...
    if (openOnly) {
      await finishOpenPhotoPrefetch(admin, reference, token, photo.failures)
      return {
        status: 200,
        payload: {
          ...staticAssetMediaPayload(null, 'open_photo_miss'),
          retryable: false,
    ...
    const diagnostics = [
      ...photo.failures.slice(0, 3),
      google.error ? `google: ${google.error}` : null
