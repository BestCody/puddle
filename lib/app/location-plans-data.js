...
export async function getLocationPlansSnapshot({ supabase, user }) {
  const [states, visits, memberships] = await Promise.all([
    queryOr(
      supabase
  ...
  const matches = deckIds.length
    ? await queryOr(
        supabase
          .from('date_match_matches')
          .select('deck_id,location_id,status,planned_for,matched_at,updated_at,locations(id,name,slug,summary,city,cover_path,status)')
          .in('deck_id', deckIds)
  ...
  for (const row of visits) {
    ...
    plannedMap.set(row.location_id, placeItem(row, { status: 'planned', planned_for: row.planned_for, plan_source: 'personal' }))
  }
  for (const row of matches) {
    if (row.status !== 'planned' || row.locations?.status !== 'published') continue
    plannedMap.set(row.location_id, placeItem(row, { status: 'planned', planned_for: row.planned_for, plan_source: 'date_match' }))
  }
  ...
  for (const row of matches) {
    ...
    pastMap.set(row.location_id, placeItem(row, { status: 'visited', visited_at: row.updated_at || row.planned_for || row.matched_at, visit_source: 'date_match' }))
  }
