...
const plansPageInFlight = new Map()
const planStatusInFlight = new Map()
...
function normalizeSavedCategory(value) {
...
function lightweightSavedItem(row) {
  return {
    id: row.location_id,
    location_id: row.location_id,
    kind: 'place',
...
function lightweightPlannedItem(row) {
  return {
    id: row.location_id,
    location_id: row.location_id,
    kind: 'place',
...
async function rawRows(session, active, cursor, requested, { category = null, query = '' } = {}) {
  if (active === 'saved') {
    return rpcPage(session, 'location_saved_page_v1', {
      ...
      category_filter: normalizeSavedCategory(category),
      search_term: normalizeSavedQuery(query)
    }, 'savedHistory')
  }
  if (active === 'planned') {
...
async function loadLocationPlansPage(session, {
  tab = 'saved',
  cursor = null,
  limit = LOCATION_HISTORY_PAGE_SIZE,
  category = null,
  query = '',
  lightweightSaved = false,
  lightweightPlanned = false
} = {}) {
  ...
  // Default Saved view is deliberately relationship-only. This lets the page render
  // from Supabase immediately; titles are hydrated in a deferred client request and
  // full catalogue details are fetched only after the user opens a card.
  if ((active === 'saved' && lightweightSaved) || (active === 'planned' && lightweightPlanned)) {
    const batch = await rawRows(session, active, decodedCursor, requested)
    const rawHasMore = batch.length >= requested
    const pageRows = batch.slice(0, size)
    const items = pageRows.map(active === 'saved' ? lightweightSavedItem : lightweightPlannedItem)
    recordSloObservation('savedHistory', elapsedMs(started), true, {
      trace_id: traceId, service: 'vercel', tab: active, page_size: size,
      returned: items.length, has_more: rawHasMore, lightweight: true
    })
    return {
      items,
      pagination: {
        pageSize: size,
        hasMore: rawHasMore,
        nextCursor: rawHasMore && pageRows.length ? nextCursor(active, pageRows[pageRows.length - 1]) : null
      }
    }
  }
  ...
  const savedFilters = active === 'saved' ? { category, query } : undefined

  for (let scan = 0; scan < HYDRATION_SCAN_LIMIT && collected.length <= size; scan += 1) {
    const batch = await rawRows(session, active, scanCursor, requested, savedFilters)
...
export async function getLocationPlansPage(session, options = {}) {
  const key = JSON.stringify([
    String(session?.user?.id || ''),
    options.tab || 'saved',
    options.cursor || null,
    Number(options.limit) || LOCATION_HISTORY_PAGE_SIZE,
    options.category || null,
    String(options.query || '').trim(),
    Boolean(options.lightweightSaved),
    Boolean(options.lightweightPlanned)
  ])
  const active = plansPageInFlight.get(key)
  if (active) return active
  const promise = loadLocationPlansPage(session, options)
  plansPageInFlight.set(key, promise)
  try {
    return await promise
  } finally {
    if (plansPageInFlight.get(key) === promise) plansPageInFlight.delete(key)
  }
}

async function loadLocationPlanStatus(session, locationId) {
...
export async function getLocationPlanStatus(session, locationId) {
  const key = `${String(session?.user?.id || '')}:${String(locationId || '')}`
  const active = planStatusInFlight.get(key)
  if (active) return active
  const promise = loadLocationPlanStatus(session, locationId)
  planStatusInFlight.set(key, promise)
  try {
    return await promise
  } finally {
    if (planStatusInFlight.get(key) === promise) planStatusInFlight.delete(key)
  }
}
...
export async function getLocationPlansMapSnapshot(session) {
  const [savedPage, plannedPage] = await Promise.all([
    getLocationPlansPage(session, { tab: 'saved', lightweightSaved: true }),
    getLocationPlansPage(session, { tab: 'planned', lightweightPlanned: true })
  ])
  return {
    saved: savedPage.items,
    planned: plannedPage.items
