...
export async function getLocationPlansSnapshot({ supabase, user }) {
  ...
  const [states, visits, memberships, perfectEvents] = await Promise.all([
    queryOr(
      supabase
        ...
        .select('state,location_id,created_at,pinned_at,locations(id,name,slug,summary,kind,city,cover_path,status)')
  ...
  const saved = [...savedMap.values()]
    ...
    .sort((a, b) => {
      ...
      return new Date(b.pinned_at || b.saved_at || 0) - new Date(a.pinned_at || a.saved_at || 0)
    })
