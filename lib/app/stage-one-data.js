const demoDiscover = [
  { id: 'demo-event-1', kind: 'EVENT', category: 'LIVE MUSIC', title: 'Neon Garden', meta: 'Friday · 10 PM · The Junction', distance: '2.1 km', badge: 'TONIGHT', symbol: '♫', accent: '#ff4fa3', tags: ['18+', '$18', '3 friends saved'], href: '/events/neon-garden' },
  { id: 'demo-place-1', kind: 'PLACE', category: 'LATE-NIGHT CAFÉ', title: 'Moonlight Café', meta: 'Open until 1 AM · espresso, vinyl, soft lights', distance: '1.2 km', badge: 'OPEN NOW', symbol: '☕', accent: '#ffd86b', tags: ['Cozy', '$$', 'Wi-Fi'], href: '/places/moonlight-cafe' },
  { id: 'demo-event-2', kind: 'EVENT', category: 'ART', title: 'Midnight Museum', meta: 'Saturday · 9 PM · West end', distance: '4.8 km', badge: 'WEEKEND', symbol: '✦', accent: '#7c4dff', tags: ['Free', 'All ages', 'Accessible'], href: '/events/neon-garden' },
  { id: 'demo-place-2', kind: 'PLACE', category: 'OUTDOORS', title: 'Sunset Steps', meta: 'Scenic lookout · best around golden hour', distance: '3.4 km', badge: 'LOCAL GEM', symbol: '☀', accent: '#72e6c1', tags: ['Free', 'Outdoors', 'Great views'], href: '/places/moonlight-cafe' }
]
...
function eventToCard(event) {
  return {
    id: event.id,
    kind: 'EVENT',
    category: String(event.category || 'EVENT').replaceAll('-', ' ').toUpperCase(),
    title: event.title,
...
export async function getStageOneSnapshot({ supabase, user }) {
  const [events, locations, states, hosts, friendships, messages] = await Promise.all([
    queryOr(supabase.from('events').select('id,slug,title,category,starts_at,price_from_cents,min_age,location_id,locations(city)').eq('status', 'published').order('starts_at').limit(8)),
    queryOr(supabase.from('locations').select('id,slug,name,kind,summary,city,neighborhood,price_level,amenities').eq('status', 'published').limit(8)),
