...
export async function ensureGlobalLocationReferences(admin, ids = []) {
  ...
  // The canonical B2 search catalogue remains authoritative even when the tiny
  // Supabase reference already exists. Returning these rows lets callers use
  // canonical coordinates transiently without copying catalogue metadata into Postgres.
