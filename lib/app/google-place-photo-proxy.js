...
async function googlePlaceDetails(placeId, { apiKey, timeoutMs }) {
  const id = String(placeId || '').trim()
  if (!id || id.length > 300) throw new Error('Google Place ID is unavailable.')
...
export async function fetchGooglePlacePhotoById(placeId, {
  apiKey,
  timeoutMs = 8_000
} = {}) {
  const key = String(apiKey || '').trim()
  if (!key) throw new Error('Google Places server key is unavailable.')
  const safeTimeout = Math.max(2_000, Math.min(15_000, Number(timeoutMs) || 8_000))
  const place = await googlePlaceDetails(placeId, { apiKey: key, timeoutMs: safeTimeout })
  const photo = choosePhoto(place?.photos)
  if (!photo) {
    const error = new Error('The verified Google place has no usable photo.')
    error.status = 404
    throw error
  }
  const media = await downloadPhoto(photo, { apiKey: key, timeoutMs: safeTimeout })
  return {
    ...media,
...
export async function fetchFreshGooglePlacePhotoForPlace(place, {
  apiKey,
  minimumScore = 0.86,
  timeoutMs = 8_000
} = {}) {
...
export async function fetchFreshGooglePlacePhoto(reference, options = {}) {
  ...
  return fetchFreshGooglePlacePhotoForPlace(place, options)
}
