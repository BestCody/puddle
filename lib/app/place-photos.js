...
export function allowedPhotoHosts(
  ...
  b2Url = process.env.B2_DOWNLOAD_BASE_URL || process.env.B2_PUBLIC_BASE_URL || process.env.STATIC_CATALOGUE_BASE_URL || process.env.NEXT_PUBLIC_CATALOGUE_ASSET_BASE_URL
) {
  ...
  const b2Host = photoAssetHost(b2Url)
  if (projectHost) hosts.add(projectHost)
  if (b2Host) hosts.add(b2Host)
  return hosts
}
...
export function photoMetadata(row) {
  ...
  return {
    ...
    storageBackend: safeText(row.storage_backend, 20) || null,
    storageKey: safeText(row.storage_key, 1_000) || null,
