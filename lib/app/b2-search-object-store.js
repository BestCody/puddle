...
export async function downloadB2SearchObject(key, {
  ...
  const promise = (async () => {
    ...
    for (let attempt = 0; attempt < 4; attempt += 1) {
      ...
      const { auth, bucketName, authorizationMode, downloadBaseUrl } = await authorization(prefix, env, fetchFn)
      const url = authorizationMode === 'download'
        ? `${downloadBaseUrl}/${encodePath(objectKey)}`
        : `${auth.downloadUrl}/file/${encodeURIComponent(bucketName)}/${encodePath(objectKey)}`
      let response
      try {
