...
let runtimeDataConfigInFlight = null
...
async function vaultDataConfig(env) {
  ...
  if (runtimeDataConfigInFlight?.env === env) return runtimeDataConfigInFlight.promise
  const promise = (async () => {
  ...
  runtimeDataConfigInFlight = { env, promise }
  try {
    return await promise
  } finally {
    if (runtimeDataConfigInFlight?.promise === promise) runtimeDataConfigInFlight = null
  }
}
...
async function authorization(prefix, env, fetchFn) {
  ...
  const cacheKey = `${String(prefix || '').toUpperCase()}:${config.keyId}:${config.bucketId || ''}:${config.bucketName || ''}`
...
export function clearB2SearchAuthorizationCache() {
  ...
  runtimeDataConfigInFlight = null
