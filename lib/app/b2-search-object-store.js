...
import { queueB2RuntimeObjectCacheWrite, readB2RuntimeObjectCache } from './b2-runtime-object-cache.js'
...
function downloadConfig(env) {
  ...
  return completeConfig(config) ? config : null
}
...
async function authorization(prefix, env, fetchFn) {
  ...
  const promise = (async () => {
    ...
    const usesDownloadAuthorization = config.authorizationMode === 'download' && !capabilities.has('readFiles')
    if (capabilities.size && !capabilities.has('readFiles') && !(usesDownloadAuthorization && capabilities.has('shareFiles'))) {
      throw new Error(`${prefix} application key requires readFiles capability or shareFiles for scoped downloads.`)
    }
    ...
    const effectiveAuth = usesDownloadAuthorization
      ...
      : auth
    const value = { auth: effectiveAuth, bucketName }
