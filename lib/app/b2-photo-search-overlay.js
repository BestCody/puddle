...
const LEGACY_OVERLAY_VERSION = 1
...
const PHOTO_OBJECT_CACHE_LIMIT = 32
...
function overlayActiveKey(env = process.env) {
  const dataPrefix = normalizedKey(env.B2_DATA_PREFIX || 'data')
  return normalizedKey(env.GLOBAL_LOCATION_PHOTO_OVERLAY_ACTIVE_KEY || `${dataPrefix}/search/photo-overlay-v1/active.json`)
}
...
function expectedLegacyObjectKey(snapshot, digest, manifest) {
  const prefix = normalizedKey(manifest?.prefix)
  if (!prefix || !prefix.endsWith(`/schema=v1/snapshot=${snapshot}`)) return ''
  return `${prefix}/photo-overlay-v1/sha256=${digest}/photos.json.br`
}

function searchPrefix(manifest) {
  const prefix = normalizedKey(manifest?.prefix)
  const marker = '/schema=v1/snapshot='
  const markerIndex = prefix.indexOf(marker)
  return markerIndex > 0 ? prefix.slice(0, markerIndex) : ''
}

function expectedShardedObjectKey(bucket, digest, manifest) {
  const prefix = searchPrefix(manifest)
  if (!prefix) return ''
  return `${prefix}/photo-overlay-v2/objects/sha256=${digest}/bucket=${bucket}.json.br`
}

function validBucketCount(value) {
  const count = Number(value)
  return Number.isInteger(count) && count >= 2 && count <= 64 && (count & (count - 1)) === 0 ? count : 0
}

function photoBucket(identifier, bucketCount) {
  const digest = createHash('sha256').update(String(identifier)).digest()
  return (digest[0] & (bucketCount - 1)).toString(16).padStart(2, '0')
}
...
function validateActive(value, manifest, manifestKey) {
  ...
  if (Number(value.schema_version) !== 1 || ![LEGACY_OVERLAY_VERSION, OVERLAY_VERSION].includes(Number(value.overlay_version))) {
    throw new Error('B2 photo overlay active pointer has an unsupported schema.')
  }
  const overlayVersion = Number(value.overlay_version)
  ...
  const bloom = parseBloom(value.bloom)
  if (overlayVersion === LEGACY_OVERLAY_VERSION) {
    const digest = String(value.object_sha256 || '').toLowerCase()
    const objectKey = normalizedKey(value.object_key)
    if (!validSha256(digest) || objectKey !== expectedLegacyObjectKey(snapshot, digest, manifest)) {
  ...
  const bucketCount = validBucketCount(value.bucket_count)
  if (!bucketCount) throw new Error('B2 photo overlay bucket_count is invalid.')
  if (!value.shards || typeof value.shards !== 'object' || Array.isArray(value.shards)) {
    throw new Error('B2 photo overlay shards are invalid.')
  }
  const shards = {}
  let shardPhotoCount = 0
  for (const [bucket, shard] of Object.entries(value.shards)) {
    if (!/^[0-9a-f]{2}$/.test(bucket) || parseInt(bucket, 16) >= bucketCount || !shard || typeof shard !== 'object' || Array.isArray(shard)) {
      throw new Error('B2 photo overlay contains an invalid shard.')
    }
    const digest = String(shard.object_sha256 || '').toLowerCase()
    const objectKey = normalizedKey(shard.object_key)
    const shardCount = Number(shard.photo_count)
    if (!validSha256(digest) || objectKey !== expectedShardedObjectKey(bucket, digest, manifest) || !Number.isInteger(shardCount) || shardCount < 1) {
      throw new Error(`B2 photo overlay shard ${bucket} identity is invalid.`)
    }
    shards[bucket] = { ...shard, objectKey, objectSha256: digest, photoCount: shardCount, overlayVersion }
    shardPhotoCount += shardCount
  }
  if (shardPhotoCount !== photoCount) throw new Error('B2 photo overlay shard counts do not match its active pointer.')
  return {
    ...value,
    overlayVersion,
    photoCount,
...
async function loadPhotoObject({ objectKey, objectSha256, photoCount, overlayVersion }, { env = process.env, fetchFn = fetch, signal } = {}) {
  const key = objectKey
  const cached = OBJECT_CACHE.get(key)
  if (cached) {
    OBJECT_CACHE.delete(key)
    OBJECT_CACHE.set(key, cached)
    return cached
  }
  ...
  const promise = (async () => {
    ...
    if (digest !== objectSha256) throw new Error('B2 photo overlay object checksum does not match its active pointer.')
    ...
    if (!Array.isArray(payload) || Number(payload[0]) !== overlayVersion || !Array.isArray(payload[1])) {
    ...
    if (photos.size !== photoCount) throw new Error('B2 photo overlay count does not match its active pointer.')
    OBJECT_CACHE.delete(key)
    OBJECT_CACHE.set(key, photos)
    const cacheLimit = integer(env.GLOBAL_LOCATION_PHOTO_OVERLAY_OBJECT_CACHE_LIMIT, PHOTO_OBJECT_CACHE_LIMIT, 2, 64)
    while (OBJECT_CACHE.size > cacheLimit) {
      const oldest = OBJECT_CACHE.keys().next().value
      OBJECT_CACHE.delete(oldest)
    }
    return photos
  })()
...
export async function getPhotoSearchOverlay({ manifest, manifestKey, ids = [], env = process.env, fetchFn = fetch, signal } = {}) {
  ...
  if (active.overlayVersion === LEGACY_OVERLAY_VERSION) {
    const allPhotos = await loadPhotoObject(active, { env, fetchFn, signal })
    const photos = new Map()
  ...
  for (const id of possible) {
    const bucket = photoBucket(id, active.bucketCount)
    if (!active.shards[bucket]) continue
    const idsForBucket = buckets.get(bucket) || []
    idsForBucket.push(id)
    buckets.set(bucket, idsForBucket)
  }
  const loaded = await mapConcurrent([...buckets.keys()], 8, async (bucket) => {
    const photos = await loadPhotoObject(active.shards[bucket], { env, fetchFn, signal })
    return { bucket, photos }
  })
  const photos = new Map()
  for (const { bucket, photos: shardPhotos } of loaded) {
    for (const id of buckets.get(bucket) || []) {
      const photo = shardPhotos.get(id)
      if (photo) photos.set(id, photo)
    }
  }
  return { active: true, photos, possibleCount: possible.length, matchedCount: photos.size, photoCount: active.photoCount }
