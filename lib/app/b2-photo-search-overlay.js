...
const PHOTO_OBJECT_CACHE_LIMIT = 2
...
function expectedObjectKey(snapshot, digest, manifest) {
...
function validateActive(value, manifest, manifestKey) {
  ...
  if (!validSha256(digest) || objectKey !== expectedObjectKey(snapshot, digest, manifest)) {
...
async function loadObject(active, { env = process.env, fetchFn = fetch, signal } = {}) {
  ...
  const promise = (async () => {
    ...
    while (OBJECT_CACHE.size > PHOTO_OBJECT_CACHE_LIMIT) {
