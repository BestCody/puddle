...
const DISCOVERY_QUERY_PAGE_SIZE = 60
...
function card(session, row) {
  ...
  return {
    ...
    google_client_lookup: googleClientLookup, google_lookup_min_score: null,
...
function infrastructure({ served = 0, googleEligible = 0, timings = null } = {}) {
  return {
    source: 'supabase-relational',
    relationalServed: served,
    googleUiKitEligible: googleEligible,
    overlayRpc: 'r2_discovery_overlay_v2',
    timings: timings || { queryMs: 0, totalMs: 0 }
  }
}
...
function emptyFeed(session, filters) {
  return {
    ...
    fallback: false, fallbackReason: null, rankingVersion: 'supabase-relational-v3',
    experiment: { experiment: 'supabase-relational-v3', variant: 'control', bucket: 0, holdout: false },
...
export async function getRelationalDiscoveryFeed(session, rawFilters = {}, { excludeIds = [] } = {}) {
  ...
  const transientExcludes = [...new Set((excludeIds || []).map(String).filter(Boolean))]
  const candidatePageSize = Math.min(100, Math.max(DISCOVERY_QUERY_PAGE_SIZE, filters.limit * 4))
  const queryStarted = performance.now()
  const result = await session.supabase.rpc('r2_discovery_overlay_v2', {
    center_lat: latitude,
    center_lng: longitude,
    radius_m: Math.round(filters.distance * 1000),
    max_rows: candidatePageSize,
    exclude_ids: transientExcludes,
    category_filter: filters.category || null,
    price_filter: /^[1-4]$/.test(filters.price) ? Number(filters.price) : null,
    query_filter: filters.q || null,
    amenity_filter: filters.amenity || null,
    accessible_only: Boolean(filters.accessible),
    open_now_only: Boolean(filters.openNow)
  })
  if (result.error) throw result.error
  const queryMs = performance.now() - queryStarted
  const overlay = result.data || { interests: [], locations: [] }
  const rawRows = overlay.locations || []
  const candidates = rawRows.filter((row) => matches(row, filters)).map((row) => card(session, row))
  ...
  return {
    ...
    recycled: false,
    emptyReason: items.length ? null : hasActiveFilters(filters) ? 'filters' : transientExcludes.length ? 'exhausted' : 'catalogue_sync_pending',
    continuation: {
      excluded: transientExcludes.length,
      candidateLimit: candidatePageSize,
      hasMore: rawRows.length >= candidatePageSize || candidates.length > items.length
    },
    fallback: false, fallbackReason: null, rankingVersion: 'supabase-relational-v3',
    experiment: { experiment: 'supabase-relational-v3', variant: 'control', bucket: 0, holdout: false },
    rejections: [], personalization: { behavioral: false, friendActivity: false, vector: false, explicitInterestsOnly: true },
    infrastructure: infrastructure({
      served: items.length,
      googleEligible: items.filter((item) => !item.photo_url && Boolean(item.google_place_id || item.google_client_lookup)).length,
