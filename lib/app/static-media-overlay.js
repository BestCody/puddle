...
import { r2Configuration, r2Request } from './r2-s3.js'
import {
...
async function readOverlay(key, config) {
  ...
  if (response.status === 404) {
    return { payload: { v: staticCatalogueSchema.mediaVersion, m: [] }, etag: null }
  }
...
async function writeOverlayWithRetry(key, updates, config, maxAttempts = 4) {
  for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
    const current = await readOverlay(key, config)
    const map = new Map(current.payload.m.filter((row) => Array.isArray(row) && row[0]).map((row) => [String(row[0]), row]))
    ...
    if (JSON.stringify(rows) === JSON.stringify(current.payload.m)) {
      return { records: rows.length, changed: false, attempts: attempt + 1 }
    }
    ...
    const response = await r2Request({ method: 'PUT', key, body, headers, config })
    if (response.status === 412) continue
    if (!response.ok) throw new Error(`R2 media overlay write failed for ${key}: ${response.status} ${await response.text()}`)
    return { records: rows.length, changed: true, attempts: attempt + 1 }
  }
  throw new Error(`R2 media overlay ${key} changed concurrently too many times.`)
}
...
export async function syncStaticMediaOverlayForLocations(admin, locationIds, {
  ...
  for (const [key, group] of grouped) {
    const result = await writeOverlayWithRetry(key, group.updates, config)
    records += result.records
    if (result.changed) changedTiles += 1
    retries += Math.max(0, result.attempts - 1)
  }

  return { locations: staticIds.size, tiles: grouped.size, records, changedTiles, retries }
}
