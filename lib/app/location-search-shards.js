...
const OBJECT_PARSE_IN_FLIGHT = new Map()
...
async function parseObject(key, { compressed = key.endsWith('.br'), missingOk = false, cacheable = true, env = process.env, fetchFn = fetch, signal } = {}) {
  ...
  const inFlightKey = `${key}:${compressed ? 'compressed' : 'plain'}:${missingOk ? 'missing' : 'required'}:${cacheable ? 'cache' : 'nocache'}:${maxObjectBytes}:${maxDecodedBytes}`
  const existing = OBJECT_PARSE_IN_FLIGHT.get(inFlightKey)
  ...
  OBJECT_PARSE_IN_FLIGHT.set(inFlightKey, { env, fetchFn, promise })
  try {
    return await promise
  } finally {
    if (OBJECT_PARSE_IN_FLIGHT.get(inFlightKey)?.promise === promise) OBJECT_PARSE_IN_FLIGHT.delete(inFlightKey)
  }
}

export function locationSearchRuntimeConfig(env = process.env) {
  const dataPrefix = String(env.B2_DATA_PREFIX || 'data').trim().replace(/^\/+|\/+$/g, '') || 'data'
  return {
    activeKey: String(env.GLOBAL_LOCATION_SEARCH_MANIFEST_KEY || `${dataPrefix}/search/active.json`).trim().replace(/^\/+/, ''),
...
export async function getActiveSearchManifest({ env = process.env, fetchFn = fetch, signal } = {}) {
  ...
  if (
    manifestInFlight?.key === config.activeKey &&
    manifestInFlight?.env === env &&
    manifestInFlight?.fetchFn === fetchFn
  ...
  manifestInFlight = { key: config.activeKey, env, fetchFn, promise }
  try {
    return await promise
  } finally {
    if (manifestInFlight?.promise === promise) manifestInFlight = null
  }
}
...
export async function resolveGeoShardPlan(bounds, { env = process.env, fetchFn = fetch, signal, manifest } = {}) {
  ...
  const routeObjects = await mapConcurrent(tiles, config.fetchConcurrency, async ([latIndex, lonIndex]) => {
    const basePrefix = `${String(resolvedManifest.prefix || '').replace(/\/+$/, '')}/routing`
    const relative = `routing/${latIndex}/${lonIndex}.json.br`
    const key = prefix === basePrefix
      ? manifestObjectKey(resolvedManifest, relative)
      : routeKey(prefix, latIndex, lonIndex)
    return parseObject(key, { compressed: true, missingOk: true, env, fetchFn, signal })
  })
...
export function manifestObjectKey(manifest, relative) {
  const normalized = String(relative || '').trim().replace(/^\/+/, '')
  if (!normalized) throw new Error('B2 search object relative path is required.')
  const artifacts = manifest?.object_index?.artifacts
  if (artifacts && Object.prototype.hasOwnProperty.call(artifacts, normalized)) {
    const key = String(artifacts[normalized] || '').trim().replace(/^\/+/, '')
    if (!key) throw new Error(`B2 search manifest contains an empty object key for ${normalized}.`)
    return key
  }
  return `${dataPrefix(manifest)}/${normalized}`
}
...
export async function getLocationsByIdsFromShards(ids, { env = process.env, fetchFn = fetch, signal, manifest } = {}) {
  ...
  const objects = await mapConcurrent(entries, config.fetchConcurrency, async ([bucket, group]) => {
    const object = await parseObject(manifestObjectKey(resolvedManifest, `id/${bucket}.json.br`), { compressed: true, missingOk: true, env, fetchFn, signal })
    if (!object || typeof object !== 'object' || Array.isArray(object)) return []
    return group.map((id) => object[id]).filter(Boolean)
  })
  const loaded = objects.flat()
  const byId = new Map()
  for (const row of loaded) if (row?.id) byId.set(String(row.id), row)
  return values.map((id) => byId.get(id)).filter(Boolean)
}
...
export async function getLocationBySlugFromShards(slug, { env = process.env, fetchFn = fetch, signal, manifest } = {}) {
  ...
  const slugMap = await parseObject(manifestObjectKey(resolvedManifest, `slug/${bucket}.json.br`), { compressed: true, missingOk: true, env, fetchFn, signal })
...
export async function fetchCoarseViewportDocuments(bounds, zoom, { env = process.env, fetchFn = fetch, signal, manifest } = {}) {
  ...
  const level = Number(zoom) < 5 ? 'z0' : 'z1'
  const basePrefix = `${String(resolvedManifest.prefix || '').replace(/\/+$/, '')}/geo-map/${level}`
  const objects = await mapConcurrent(tiles, config.fetchConcurrency, ([latIndex, lonIndex]) => {
    const relative = `geo-map/${level}/${latIndex}/${lonIndex}.json.br`
    const key = prefix === basePrefix
      ? manifestObjectKey(resolvedManifest, relative)
      : `${prefix}/${latIndex}/${lonIndex}.json.br`
    return parseObject(key, { compressed: true, missingOk: true, env, fetchFn, signal })
  })
  const byId = new Map()
