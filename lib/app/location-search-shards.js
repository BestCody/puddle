...
import { queueB2RuntimeLocationCacheWrite, readB2RuntimeLocationCache } from './b2-runtime-object-cache.js'
...
export async function getLocationsByIdsFromShards(ids, { env = process.env, fetchFn = fetch, signal, manifest } = {}) {
  ...
  const cached = await readB2RuntimeLocationCache(prefix, values, { env, fetchFn })
  const missing = values.filter((id) => !cached.has(id))
  if (!missing.length) return values.map((id) => cached.get(id)).filter(Boolean)
  ...
  queueB2RuntimeLocationCacheWrite(prefix, loaded, { env, fetchFn })
