...
async function parseObject(key, { compressed = key.endsWith('.br'), missingOk = false, env = process.env, fetchFn = fetch, signal } = {}) {
  const cached = cacheGet(key)
  if (cached !== null) return cached
  const maxObjectBytes = integer(env.GLOBAL_LOCATION_MAX_OBJECT_BYTES, 4 * 1024 * 1024, 64 * 1024, 32 * 1024 * 1024)
  const body = await downloadB2SearchObject(key, { env, fetchFn, maxBytes: maxObjectBytes, signal, missingOk })
  ...
  cacheSet(key, value, body.length + raw.length, env)
  return value
}
...
export function locationSearchRuntimeConfig(env = process.env) {
  return {
    ...
    timeoutMs: integer(env.GLOBAL_LOCATION_SEARCH_TIMEOUT_MS, 5000, 750, 15_000),
    ...
    fetchConcurrency: integer(env.GLOBAL_LOCATION_FETCH_CONCURRENCY, 8, 1, 24)
  }
}
...
export async function getActiveSearchManifest({ env = process.env, fetchFn = fetch, signal } = {}) {
  ...
  const active = await parseObject(config.activeKey, { compressed: false, env, fetchFn, signal })
...
export function radiusBoundingBox(latitude, longitude, distanceKm) {
  ...
  if (north >= 89.999 || south <= -89.999) return { north, south, west: -180, east: 180 }
  const cos = Math.max(0.01, Math.cos((lat * Math.PI) / 180))
  const lonDelta = Math.min(180, radius / (111.320 * cos))
  if (lonDelta >= 180) return { north, south, west: -180, east: 180 }
  return { north, south, west: wrapLongitude(lon - lonDelta), east: wrapLongitude(lon + lonDelta) }
}
...
function descriptorBounds(descriptor) {
  ...
  return { north: Number(descriptor.north), south: Number(descriptor.south), east: Number(descriptor.east), west: Number(descriptor.west) }
}
...
export async function resolveGeoShardPlan(bounds, { env = process.env, fetchFn = fetch, signal, manifest } = {}) {
  ...
  if (tiles.length > config.maxDirectoryTiles) throw new RangeError(`Location query intersects ${tiles.length} routing tiles; maximum is ${config.maxDirectoryTiles}.`)
  ...
  for (const route of routeObjects) {
    ...
    for (const descriptor of route) {
      ...
      if (!key || shards.has(key) || !boundsOverlap(bounds, descriptorBounds(descriptor))) continue
      const bytes = descriptorBytes(descriptor)
      const count = descriptorCount(descriptor)
      shards.set(key, { key, bytes, count })
      compressedBytes += bytes
      candidateCount += count
    }
  }
  if (shards.size > config.maxShards) throw new RangeError(`Location query requires ${shards.size} shards; maximum is ${config.maxShards}.`)
  ...
  return { shards: [...shards.values()], compressedBytes, candidateCount, routingTiles: tiles.length }
}
...
export async function fetchCoarseViewportDocuments(bounds, zoom, { env = process.env, fetchFn = fetch, signal, manifest } = {}) {
  ...
  const tiles = directoryTilesForBounds(bounds, degrees)
  const config = locationSearchRuntimeConfig(env)
