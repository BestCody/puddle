...
async function hydrateLocations(session, ids) {
  ...
  const rows = await Promise.all(uniqueIds.map((id) => cachedSocialLocation(id)))
...
const cachedSocialLocation = unstable_cache(
  async (id) => {
    const rows = await getGlobalLocationsByIds([id], { traceId: null })
    if (rows.length !== 1) throw new Error(`Global location ${id} is missing from the active B2 catalogue.`)
    return rows[0]
  },
  ['social-feed-location-hydration-v2'],
