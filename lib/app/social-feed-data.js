...
async function queryCommentPreviews(session, postIds, traceId) {
  ...
  // Schema-drift fallback. The timestamped restore migration makes the indexed RPC
  // authoritative, but this keeps feed rendering safe while a deployment catches up.
  const fallbackStarted = latencyStart()
  ...
  try {
    const { data, error } = await session.supabase
      ...
      .order('created_at', { ascending: false })
      .limit(limit)
    recordSupabaseQuery('social_comment_previews_fallback', fallbackStarted, traceId, Boolean(error))
    return error ? [] : data || []
  } catch {
    recordSupabaseQuery('social_comment_previews_fallback', fallbackStarted, traceId, true)
    return []
  }
}
