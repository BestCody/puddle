...
const globalCircuit = { failures: [], openUntil: 0 }
...
function markCached(feed, degraded = false, reason = null) {
  return {
    ...feed,
    ...
    infrastructure: {
      ...(feed.infrastructure || {}),
      source: degraded ? 'global-location-stale-cache' : 'global-location-cache',
...
function emptyDegradedFeed(session, filters, reason) {
  ...
  return {
    requestId: randomUUID(), items: [], filters,
    center: hasCenter ? { latitude, longitude } : null,
    centerLabel: session.profile?.location_label || session.profile?.city || null,
    categories: [], recycled: false, emptyReason: 'temporarily_unavailable',
    continuation: { excluded: 0, candidateLimit: 0, hasMore: false },
    fallback: true, fallbackReason: reason, rankingVersion: 'global-location-v1',
    ...
    infrastructure: {
      source: 'global-location-degraded', requestedSource: 'global-location-serving',
      circuitOpen: circuitIsOpen(), candidates: 0, timings: { queryMs: 0, totalMs: 0 }
    }
  }
}

async function controlledFailureResponse(session, filters, key, reason, error) {
  const stale = cachedFeed(key, STALE_TTL_MS)
  if (stale) return withoutModeratedLocations(session, markCached(stale, true, `${reason}_stale_cache`))
  console.error(`Global location serving degraded: ${error?.message || reason}`)
...
export async function getDiscoveryFeed(session, filters = {}, options = {}) {
  ...
  const now = Date.now()
  trimCache(now)
  if (circuitIsOpen(now)) return controlledFailureResponse(session, filters, key, 'global_location_circuit_open', null)

  const fresh = cachedFeed(key, SUCCESS_TTL_MS, now)
  if (fresh) return withoutModeratedLocations(session, markCached(fresh))

  try {
    const feed = await withoutModeratedLocations(session, await getGlobalDiscoveryFeed(session, filters, options))
    recordSuccess()
    rememberSuccess(key, feed)
    return feed
  } catch (error) {
    recordFailure()
    return controlledFailureResponse(session, filters, key, 'global_location_serving_failure', error)
  }
}
