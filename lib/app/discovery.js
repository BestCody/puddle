...
function allowRelationalFallback(env = process.env, now = Date.now()) {
  // Two switches are deliberate. GLOBAL_LOCATION_FALLBACK_TO_SUPABASE existed before
  // the circuit breaker, so a stale production value of `true` must not silently
  // reactivate Postgres failover. Operators must explicitly enable emergency mode too.
  const legacyEnabled = String(env.GLOBAL_LOCATION_FALLBACK_TO_SUPABASE || 'false').toLowerCase() === 'true'
  const emergencyEnabled = String(env.GLOBAL_LOCATION_EMERGENCY_RELATIONAL_FALLBACK || 'false').toLowerCase() === 'true'
  if (!legacyEnabled || !emergencyEnabled) return false
...
async function controlledFailureResponse(session, filters, options, key, reason, error) {
  ...
  if (allowRelationalFallback(process.env, now)) {
    console.error(`Global location serving failed; one rate-limited emergency relational fallback was admitted: ${error?.message || reason}`)
    try {
      ...
      console.error(`Emergency relational discovery fallback also failed: ${fallbackError?.message || 'unknown error'}`)
    }
  }
