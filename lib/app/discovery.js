...
async function directCandidates(session, center) {
  ...
  const places = (locationsResult.data || []).filter((item) => !blocked.has(item.created_by) && !item.has_private_address).map((location) => ({
    ...
    starts_at: null, ends_at: null, timezone: location.timezone, price_cents: null, price_level: location.price_level, min_age: null, capacity: null, remaining_capacity: null,
...
async function queryCandidates(session, filters) {
  const center = { latitude: filters.latitude ?? session.profile?.latitude ?? DEFAULT_CENTER.latitude, longitude: filters.longitude ?? session.profile?.longitude ?? DEFAULT_CENTER.longitude }
...
async function loadLocationPhotos(session, candidates, now) {
  const ids = [...new Set(candidates.filter((candidate) => candidate.content_kind === 'place').map((candidate) => candidate.content_id).filter(Boolean))]
...
function formatCandidate(session, candidate, ranking, locationPhotos) {
...
export async function getDiscoveryFeed(session, rawFilters = {}) {
  ...
  const locationPhotos = await loadLocationPhotos(session, candidates, now)
  const dismissed = new Set((dismissedResult.data || []).map((item) => item.event_id ? `event:${item.event_id}` : `place:${item.location_id}`))
  const normalized = candidates.map((candidate) => ({ ...candidate, open_now: candidate.content_kind === 'place' ? isOpenAt(candidate.opening_hours, candidate.timezone, now) : false }))
  const evaluated = normalized.map((candidate) => {
    const rejectionReasons = candidateRejectionReasons(candidate, filters, session.profile, now)
    if (dismissed.has(`${candidate.content_kind}:${candidate.content_id}`)) rejectionReasons.push('previously_dismissed')
    return { candidate, rejectionReasons: [...new Set(rejectionReasons)] }
  })
  const ranked = evaluated
    .filter((entry) => entry.rejectionReasons.length === 0)
    .map(({ candidate }) => formatCandidate(session, candidate, scoreHybridCandidate(candidate, contextResult.context, { filters, experiment: contextResult.experiment, requestId, now }), locationPhotos))
    .sort((a, b) => b.score - a.score || Number(b.has_real_photo) - Number(a.has_real_photo) || String(a.title).localeCompare(String(b.title)))

  const items = diversifyRecommendations(ranked, filters.limit, contextResult.context.rankingConfig?.diversity || {})
  const rejections = evaluated.filter((entry) => entry.rejectionReasons.length).slice(0, 300).map(({ candidate, rejectionReasons }) => ({ content_kind: candidate.content_kind, content_id: candidate.content_id, rejectionReasons }))
...
export async function logDiscoveryImpressions(session, feed) {
  ...
  if (!requestResult.error) {
    ...
    const candidateRows = feed.items.slice(0, 100).filter((item) => !existingTargets.has(`${item.content_kind}:${item.content_id}`)).map((item, index) => ({
      ...
      eligibility: { eligible: true, filters: feed.filters },
