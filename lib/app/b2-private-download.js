...
export function managedB2ObjectKey(key) {
  const normalized = String(key || '').replace(/^\/+/, '')
  if (!normalized || normalized.length > 2_048 || !managedB2PrefixForKey(normalized)) return null
  if (normalized.includes('..') || normalized.includes('\\') || normalized.includes('?') || normalized.includes('#')) return null
  return normalized
}
...
export function b2DownloadUrlForKey(key, config = b2PrivateDownloadConfiguration()) {
  const normalized = managedB2ObjectKey(key)
...
async function requestDownloadAuthorization(key, { config, fetchImpl, now, retry = true }) {
  ...
  if (!response.ok) {
    if (retry && response.status === 401 && ['bad_auth_token', 'expired_auth_token'].includes(payload?.code)) {
      ...
      return requestDownloadAuthorization(key, { config, fetchImpl, now, retry: false })
    }
...
export async function getB2DownloadAuthorization(key, {
  ...
  const normalized = managedB2ObjectKey(key)
  if (!normalized) throw new Error('The requested Backblaze B2 object is not allowed.')
  const cacheKey = `${config.keyId}:${config.bucket}:${normalized}:${config.tokenTtlSeconds}`
  ...
  const promise = requestDownloadAuthorization(normalized, { config, fetchImpl, now })
    .then((authorization) => {
...
export async function authorizeB2DownloadUrl(value, {
  ...
  if (!key) return value
  const authorization = await getB2DownloadAuthorization(key, { config, fetchImpl, now })
