...
const PROJECTION_PAYLOAD_CACHE_LIMIT = 64
...
function cachedProjectionPayload(key) {
  const value = PROJECTION_PAYLOAD_CACHE.get(key)
  if (!value) return null
  // Refresh insertion order so frequently used objects remain resident.
  PROJECTION_PAYLOAD_CACHE.delete(key)
  PROJECTION_PAYLOAD_CACHE.set(key, value)
  return value
}

function cacheProjectionPayload(key, payload) {
  PROJECTION_PAYLOAD_CACHE.delete(key)
  PROJECTION_PAYLOAD_CACHE.set(key, payload)
  while (PROJECTION_PAYLOAD_CACHE.size > PROJECTION_PAYLOAD_CACHE_LIMIT) {
...
async function fetchProjectionPayload(key, { env, fetchFn, signal } = {}) {
  ...
  const promise = (async () => {
    ...
    if (useMemoryCache) cacheProjectionPayload(key, payload)
    return payload
  })()
