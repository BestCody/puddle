...
const DEFAULT_PROJECTION_CACHE_BYTES = 64 * 1024 * 1024
const READY_CACHE = new Map()
const READY_IN_FLIGHT = new Map()
const PROJECTION_PAYLOAD_CACHE = new Map()
const PROJECTION_PAYLOAD_IN_FLIGHT = new Map()
let projectionPayloadCacheBytes = 0
...
async function loadProjectionReady(manifest, manifestKey, { env, fetchFn, signal } = {}) {
  ...
  const existing = READY_IN_FLIGHT.get(cacheKey)
  ...
  READY_IN_FLIGHT.set(cacheKey, { env, fetchFn, promise })
  try {
    return await promise
  } finally {
    if (READY_IN_FLIGHT.get(cacheKey)?.promise === promise) READY_IN_FLIGHT.delete(cacheKey)
  }
}
...
function projectionPayloadCacheEnabled(env, fetchFn) {
  return env.VERCEL_ENV === 'production' && fetchFn === globalThis.fetch
}

function projectionCacheLimit(env) {
  return integer(env.GLOBAL_LOCATION_PROJECTION_CACHE_BYTES, DEFAULT_PROJECTION_CACHE_BYTES, 4 * 1024 * 1024, 256 * 1024 * 1024)
}

function cachedProjectionPayload(key) {
  const entry = PROJECTION_PAYLOAD_CACHE.get(key)
  if (!entry) return null
  // Refresh insertion order so frequently used objects remain resident.
  PROJECTION_PAYLOAD_CACHE.delete(key)
  PROJECTION_PAYLOAD_CACHE.set(key, entry)
  return entry.payload
}

function cacheProjectionPayload(key, payload, bytes, env) {
  const size = Math.max(1, Number(bytes) || 1)
  const limit = projectionCacheLimit(env)
  if (size > limit / 2) return
  const existing = PROJECTION_PAYLOAD_CACHE.get(key)
  if (existing) projectionPayloadCacheBytes -= existing.bytes
  PROJECTION_PAYLOAD_CACHE.delete(key)
  PROJECTION_PAYLOAD_CACHE.set(key, { payload, bytes: size })
  projectionPayloadCacheBytes += size
  while (projectionPayloadCacheBytes > limit && PROJECTION_PAYLOAD_CACHE.size) {
    const oldest = PROJECTION_PAYLOAD_CACHE.keys().next().value
    const entry = PROJECTION_PAYLOAD_CACHE.get(oldest)
    PROJECTION_PAYLOAD_CACHE.delete(oldest)
    projectionPayloadCacheBytes -= entry?.bytes || 0
  }
}

async function fetchProjectionPayload(key, { env, fetchFn, signal } = {}) {
  const useMemoryCache = projectionPayloadCacheEnabled(env, fetchFn)
  if (useMemoryCache) {
    const cached = cachedProjectionPayload(key)
    if (cached) return cached
  }
  const existing = PROJECTION_PAYLOAD_IN_FLIGHT.get(key)
  if (existing?.env === env && existing?.fetchFn === fetchFn) return existing.promise
  ...
  const promise = (async () => {
    ...
    if (useMemoryCache) cacheProjectionPayload(key, payload, body.length + raw.length, env)
    return payload
  })()
  PROJECTION_PAYLOAD_IN_FLIGHT.set(key, { env, fetchFn, promise })
  try {
    return await promise
  } finally {
    if (PROJECTION_PAYLOAD_IN_FLIGHT.get(key)?.promise === promise) PROJECTION_PAYLOAD_IN_FLIGHT.delete(key)
  }
}
...
export function clearTextProjectionCaches() {
  READY_CACHE.clear()
  READY_IN_FLIGHT.clear()
  PROJECTION_PAYLOAD_CACHE.clear()
  PROJECTION_PAYLOAD_IN_FLIGHT.clear()
  projectionPayloadCacheBytes = 0
}
