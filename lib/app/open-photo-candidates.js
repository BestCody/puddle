...
export function bearingDegrees(fromLat, fromLng, toLat, toLng) {
  ...
  const x = Math.cos(lat1) * Math.sin(second) - Math.sin(lat1) * Math.cos(second) * Math.cos(lng2 - lng1)
...
export function streetCandidateScore({ location, image, maxDistanceM = 30, maxHeadingError = 35 }) {
  ...
  if (distanceM === null || distanceM > maxDistanceM || headingError === null || headingError > maxHeadingError) return null

  const distanceScore = 1 - Math.min(1, distanceM / maxDistanceM)
  const headingScore = 1 - Math.min(1, headingError / maxHeadingError)
  ...
  const score = distanceScore * 0.45 + headingScore * 0.4 + freshnessScore * 0.1 + landscapeScore * 0.05
...
export function commonsCandidateScore({ location, image, maxDistanceM = 500 }) {
  ...
  if (nameScore < 0.5) return null
  const distanceScore = 1 - Math.min(1, distanceM / maxDistanceM)
  const landscapeScore = Number(image?.width || 0) >= Number(image?.height || 0) ? 1 : 0.5
  return { score: nameScore * 0.72 + distanceScore * 0.23 + landscapeScore * 0.05, distanceM, nameScore }
}
