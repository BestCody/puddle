...
export async function getCurrentUser() {
  ...
  const { data: profile } = await supabase
    .from('profiles')
    .select('id, display_name, username, birth_date, city, search_radius_km, bio, avatar_path, latitude, longitude, profile_visibility, onboarding_completed_at, suspended_at, role, interests')
    .eq('id', user.id)
    .maybeSingle()
