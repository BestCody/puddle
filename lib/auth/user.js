...
export async function requireUser({ onboarding = false } = {}) {
  ...
  if (!result.configured) {
    redirect(pathWithMessage('/', 'error', 'Accounts are temporarily unavailable. Please try again later.'))
  }
  if (!result.user) redirect('/?next=/discover')
  if (result.profileError) {
    redirect(pathWithMessage('/', 'error', 'We signed you in, but could not load your profile. Please try again.'))
  }
  if (result.profile?.suspended_at || result.profile?.banned_at) {
    await result.supabase.auth.signOut()
    redirect(pathWithMessage('/', 'error', result.profile?.banned_at
