...
export function cspValue({ nonce }) {
  const supabase = cleanOrigin(process.env.NEXT_PUBLIC_SUPABASE_URL)
  const connect = ["'self'", 'https://challenges.cloudflare.com', supabase].filter(Boolean).join(' ')
  return [
    "default-src 'self'",
...
export function applySecurityHeaders(response, { nonce, request }) {
