...
export function cspValue({ nonce, staticScripts = false }) {
  ...
  const catalogue = cleanOrigin(process.env.NEXT_PUBLIC_CATALOGUE_ASSET_BASE_URL || process.env.STATIC_CATALOGUE_BASE_URL)
  ...
  const images = ["'self'", 'data:', 'blob:', 'https:', catalogue].filter(Boolean).join(' ')
  const media = ["'self'", 'blob:', 'https:', catalogue].filter(Boolean).join(' ')
