...
export function cspValue({ nonce, staticScripts = false, allowSameOriginFrame = false }) {
  ...
  const supabaseWebsocket = websocketOrigin(process.env.NEXT_PUBLIC_SUPABASE_URL)
  const catalogue = cleanOrigin(
    process.env.GLOBAL_LOCATION_SEARCH_CDN_BASE_URL || process.env.B2_DOWNLOAD_BASE_URL || process.env.STATIC_CATALOGUE_BASE_URL || process.env.B2_PUBLIC_BASE_URL || process.env.NEXT_PUBLIC_CATALOGUE_ASSET_BASE_URL
  )
  ...
  const connect = ["'self'", 'https://challenges.cloudflare.com', supabase, supabaseWebsocket, catalogue, ...googleConnections].filter(Boolean).join(' ')
  ...
  const scripts = staticScripts
    ? `script-src 'self'${developmentEval} ${scriptHosts}`
    : `script-src 'self'${developmentEval} 'nonce-${nonce}' 'strict-dynamic' ${scriptHosts}`
  return [
    "default-src 'self'",
    scripts,
