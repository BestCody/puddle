...
export async function updateSession(request, requestHeaders = request.headers, { loadProfileState = false } = {}) {
  ...
  const supabase = createServerClient(url, publishableKey, {
    ...
    cookies: {
      ...
      setAll(cookiesToSet) {
        cookiesToSet.forEach(({ name, value, options }) => request.cookies.set(name, value, authCookieSetOptions(options)))
        response = NextResponse.next({ request: { headers: requestHeaders } })
        cookiesToSet.forEach(({ name, value, options }) => response.cookies.set(name, value, authCookieSetOptions(options)))
      }
    }
  })
