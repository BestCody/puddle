...
export function stripeMembershipConfigured() {
  return Boolean(configuredValue('STRIPE_SECRET_KEY') && configuredValue('STRIPE_TINDER_PRICE_ID'))
}
...
export function membershipFromSubscription(subscription, userId = null) {
  const status = String(subscription?.status || 'inactive')
  const metadataUserId = String(subscription?.metadata?.puddle_user_id || userId || '').trim() || null
  return {
    user_id: metadataUserId,
    tier: ACTIVE_STATUSES.has(status) ? 'tinder' : 'free',
    status,
