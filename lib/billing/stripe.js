...
function cancellationIsScheduled(subscription, now = Date.now()) {
  if (subscription?.cancel_at_period_end === true) return true
  const cancelAt = Number(subscription?.cancel_at)
  return Number.isFinite(cancelAt) && cancelAt > Math.floor(now / 1000)
}
...
export function membershipFromSubscription(
  ...
  return {
    ...
    cancel_at_period_end: cancellationIsScheduled(subscription, now),
