...
export const ACTIVE_PRODUCT_SURFACES = Object.freeze([
  ...
  'DateMatch, Group Hangout Match, and shared location choice',
...
const LEGACY_PAGE_REDIRECTS = Object.freeze([
  ...
  { prefix: '/admin/finance', destination: '/admin' }
])

const LEGACY_API_EXACT_PATHS = new Set([
  '/api/drafts/event'
])
...
export function isLegacyApiPath(pathname) {
  return LEGACY_API_EXACT_PATHS.has(pathname) || LEGACY_API_PREFIXES.some((prefix) => matchesPrefix(pathname, prefix))
}