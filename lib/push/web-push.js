...
function vapidConfig() {
  ...
  return {
    publicKey,
    privateKey,
    subject: String(process.env.VAPID_SUBJECT || 'mailto:admin@puddle.you').slice(0, 240)
  }
}
...
function vapidAuthorization(endpoint, config) {
  ...
  const key = createPrivateKey({
    key: {
      kty: 'EC',
      crv: 'P-256',
      x: encodeBase64Url(config.publicKey.subarray(1, 33)),
      y: encodeBase64Url(config.publicKey.subarray(33, 65)),
