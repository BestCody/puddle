...
function pdfLooksSafe(buffer) {
  ...
  return !['/javascript', '/js ', '/launch', '/embeddedfile'].some((token) => head.includes(token))
}
...
export async function processMediaFile(file, purpose) {
  ...
  if (policy.kind === 'image') {
    if (!IMAGE_MIMES.has(mime) || (declared && !IMAGE_MIMES.has(declared))) throw new Error('Upload a JPEG, PNG, WebP, or AVIF image.')
  ...
  if (!DOCUMENT_PURPOSES.has(purpose) || mime !== 'application/pdf' || (declared && declared !== 'application/pdf')) {
    throw new Error('Verification documents must be PDF files.')
  }
  if (!pdfLooksSafe(source)) throw new Error('This PDF contains unsupported active content.')
  return {
    buffer: source,
